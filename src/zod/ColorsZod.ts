...
export const refinedMotoColorSchema = colorBaseSchema
  .refine((data) => !(data.type === "BITONO" || data.type === "PATRON") || !!data.colorTwo, {
    message: "Se requiere Color 2 para los tipos 'BITONO' y 'PATRON'.",
    path: ["colorTwo"],
  })
  .refine((data) => !(data.type === "SOLIDO" && data.colorTwo), {
    message: "No se debe proporcionar Color 2 para el tipo 'SOLIDO'.",
    path: ["colorTwo"],
  });
...
export const createColorSchema = colorBaseSchema
  ...
  .refine(
    (data) =>
      data.type === "SOLIDO" ? data.colorTwo === null || data.colorTwo === undefined : true,
    {
...
export const updateColorActionSchema = colorBaseSchema
  .extend({
