import { z } from 'zod';
import type { CurrentAccount, Client, Motorcycle, Model } from '@prisma/client';
...
export const baseCurrentAccountObject = z.object({
  ...
  totalAmount: z.number({ required_error: "El monto total es requerido." }).positive({ message: "El monto total debe ser positivo." }),
  downPayment: z.number({ required_error: "El pago inicial es requerido." }).nonnegative({ message: "El pago inicial no puede ser negativo." }),
  numberOfInstallments: z.number({ required_error: "El número de cuotas es requerido." }).int().positive({ message: "El número de cuotas debe ser un entero positivo." }),
  installmentAmount: z.number({ required_error: "El monto de la cuota es requerido." }).positive({ message: "El monto de la cuota debe ser positivo." }),
  paymentFrequency: paymentFrequencySchema,
  startDate: z.string({ required_error: "La fecha de inicio es requerida." }).datetime({ message: "Fecha de inicio inválida." }),
  reminderLeadTimeDays: z.number().int().optional(),
  status: currentAccountStatusSchema.default('ACTIVE').optional(),
  notes: z.string().optional(),
  organizationId: z.string({ required_error: "El ID de la organización es requerido." }).cuid({ message: "ID de organización inválido." }),
});

export const createCurrentAccountSchema = z.object({
  ...
  currency: z.string().min(3, "La moneda debe tener al menos 3 caracteres (ej. ARS).").max(3, "La moneda no puede tener más de 3 caracteres.").optional(),
  reminderLeadTimeDays: z.number().int().min(0, "Los días de aviso no pueden ser negativos.").optional(),
  status: currentAccountStatusSchema.optional(),
  notes: z.string().optional(),
}).refine(
  (data) => data.downPayment <= data.totalAmount, {
...
export const recordPaymentSchema = z.object({
  ...
  amountPaid: z.number({ required_error: "El monto pagado es requerido." }).positive({ message: "El monto pagado debe ser positivo." }),
  paymentDate: z.string({ required_error: "La fecha de pago es requerida." }).datetime({ message: "Fecha de pago inválida." }).optional(),
  paymentMethod: z.string().min(1, {message: "El método de pago es requerido."}).optional(),
  transactionReference: z.string().optional(),
  notes: z.string().optional(),
  isDownPayment: z.boolean().optional(),
  installmentNumber: z.number().int().positive("El número de cuota debe ser un entero positivo.").optional(),
