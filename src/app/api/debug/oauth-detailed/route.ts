  ...
  try {
    ...
    const detailedStatus = {
      ...
      globalConfig: {
        ...
        environment: process.env.MERCADOPAGO_ACCESS_TOKEN?.startsWith("TEST-")
          ? "sandbox"
          : "production",
      },
      ...
      // Diagnóstico
      diagnosis: {
        canProcessDirectPayments: !!(
          process.env.MERCADOPAGO_ACCESS_TOKEN && process.env.MERCADOPAGO_PUBLIC_KEY
        ),
        canUseOAuth: !!(process.env.MERCADOPAGO_CLIENT_ID && process.env.MERCADOPAGO_CLIENT_SECRET),
        organizationConnected: !!oauthConfig?.accessToken,
        tokenStatus: tokenValidation?.isValid
          ? "VALID"
          : tokenValidation
            ? "INVALID"
            : "NOT_CHECKED",
