...
export async function GET(request: NextRequest) {
  try {
    ...
    const mercadopagoConfig = {
      ACCESS_TOKEN: {
        ...
        value: process.env.MERCADOPAGO_ACCESS_TOKEN ? 
          `${process.env.MERCADOPAGO_ACCESS_TOKEN.substring(0, 10)}...` : 'NO CONFIGURADO'
      },
      PUBLIC_KEY: {
        ...
        value: process.env.MERCADOPAGO_PUBLIC_KEY ? 
          `${process.env.MERCADOPAGO_PUBLIC_KEY.substring(0, 10)}...` : 'NO CONFIGURADO'
      },
      CLIENT_ID: {
        present: !!process.env.MERCADOPAGO_CLIENT_ID,
        value: process.env.MERCADOPAGO_CLIENT_ID ? 
          `${process.env.MERCADOPAGO_CLIENT_ID.substring(0, 8)}...` : 'NO CONFIGURADO'
      },
      CLIENT_SECRET: {
        present: !!process.env.MERCADOPAGO_CLIENT_SECRET,
        value: process.env.MERCADOPAGO_CLIENT_SECRET ? 
          `${process.env.MERCADOPAGO_CLIENT_SECRET.substring(0, 8)}...` : 'NO CONFIGURADO'
      }
    };
    ...
    const integrationStatus = {
      canDirectPayments: mercadopagoConfig.ACCESS_TOKEN.present && mercadopagoConfig.PUBLIC_KEY.present,
      ...
      httpsAvailable: urlConfig.CURRENT_PROTOCOL === 'https' || urlConfig.BASE_URL?.startsWith('https')
    };
    ...
    return NextResponse.json({
      ...
      recommendations: {
        ...
        next_steps: integrationStatus.canDirectPayments ? 
          ...
          ['Configurar ACCESS_TOKEN + PUBLIC_KEY', 'Luego configurar CLIENT_ID + CLIENT_SECRET para OAuth']
      }
    });
