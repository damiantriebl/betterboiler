...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔍 [FORCE-PUBLIC-KEY] Intentando obtener public key para organización:', session.user.organizationId);
    ...
    try {
      ...
      if (userResponse.ok) {
        ...
        console.error('❌ [FORCE-PUBLIC-KEY] Error obteniendo user info:', userResponse.status, errorData);
      }
    } catch (error) {
      attempts.push({
        endpoint: '/users/me',
    ...
    if (userInfo?.id) {
      try {
        ...
        if (appsResponse.ok) {
          ...
          console.log('📱 [FORCE-PUBLIC-KEY] Apps data completa:', JSON.stringify(appsData, null, 2));
          ...
          const currentApp = Array.isArray(appsData) ? appsData.find((app: any) => 
          ...
          console.error('❌ [FORCE-PUBLIC-KEY] Error obteniendo apps:', appsResponse.status, errorData);
        }
      } catch (error) {
        attempts.push({
          endpoint: `/users/${userInfo.id}/mercadopago_account/applications`,
      ...
      if (!publicKey) {
        try {
          ...
          console.log('🔐 [FORCE-PUBLIC-KEY] Credentials response status:', credentialsResponse.status);

          if (credentialsResponse.ok) {
            const credentials = await credentialsResponse.json();
            console.log('🔐 [FORCE-PUBLIC-KEY] Credentials completa:', JSON.stringify(credentials, null, 2));
            ...
            console.error('❌ [FORCE-PUBLIC-KEY] Error obteniendo credentials:', credentialsResponse.status, errorData);
          }
        } catch (error) {
          attempts.push({
            endpoint: `/users/${userInfo.id}/mercadopago_credentials`,
