import { generateCurrentAccountPDF, createCurrentAccountPDFResponse } from '@/lib/pdf-generators/current-account-pdf';
...
export async function GET(
  ...
  try {
    ...
    if (!id) {
      return new Response(
        JSON.stringify({ error: 'ID de cuenta requerido' }),
        { 
          status: 400,
    ...
    if (!accountData) {
      return new Response(
        JSON.stringify({ error: 'Cuenta corriente no encontrada' }),
        { 
          status: 404,
          headers: { 'Content-Type': 'application/json' }
        }
