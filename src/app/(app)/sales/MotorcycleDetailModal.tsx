...
import { type Motorcycle, MotorcycleState } from "@prisma/client"; // Importar de Prisma
...
export function MotorcycleDetailModal({
  ...
  const renderModalActions = () => {
    ...
    if (isReservado) {
      return (
        <>
          <Button variant="outline" size="sm" onClick={() => onCancelProcess(motoIdNumber)}>
            Quitar Reserva
          </Button>
          <Button
            variant="outline"
    ...
    return (
      <>
        ...
        <Button
          ...
          size="sm"
          className="text-red-600 border-red-600 hover:bg-red-100"
          onClick={() => onSetEliminado(motoIdNumber)}
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 py-4">
          ...
          <div>
            <h3 className="text-lg font-semibold mb-2">
              Moto ID: {motorcycle.id} ({motorcycle.year})
            </h3>
            <div className="space-y-1 text-sm">
              <p>
                <span className="font-medium text-muted-foreground">Precio Venta:</span>{" "}
                {formatPrice(motorcycle.retailPrice)}
              </p>
              <p>
                <span className="font-medium text-muted-foreground">Estado Venta:</span>
                <Badge
                  variant="outline"
                  className={cn(
                    "ml-2 font-normal whitespace-nowrap",
                    estadoVentaConfig[motorcycle.state]?.className,
                  )}
                >
              ...
              <p>
                <span className="font-medium text-muted-foreground">Cilindrada:</span>{" "}
                {motorcycle.displacement ?? "N/A"}cc
              </p>
              <p>
              ...
              <p>
                <span className="font-medium text-muted-foreground">Año:</span> {motorcycle.year}
              </p>
              <p>
                <span className="font-medium text-muted-foreground">Nro Chasis:</span>{" "}
                {motorcycle.chassisNumber}
              </p>
              <p>
                <span className="font-medium text-muted-foreground">Nro Motor:</span>{" "}
                {motorcycle.engineNumber ?? "N/A"}
              </p>
              <p>
                <span className="font-medium text-muted-foreground">Patente:</span>{" "}
              ...
              <p>
                <span className="font-medium text-muted-foreground">Precio Costo:</span>{" "}
                {motorcycle.costPrice ? formatPrice(motorcycle.costPrice) : "N/A"}
              </p>
              <p>
                <span className="font-medium text-muted-foreground">Precio Mayorista:</span>{" "}
                {motorcycle.wholesalePrice ? formatPrice(motorcycle.wholesalePrice) : "N/A"}
              </p>
          ...
          <div className="flex flex-col justify-between">
            ...
            <div className="mt-auto pt-4 border-t">
              ...
              <div className="flex flex-wrap gap-2">{renderModalActions()}</div>
            </div>
          </div>
        </div>
