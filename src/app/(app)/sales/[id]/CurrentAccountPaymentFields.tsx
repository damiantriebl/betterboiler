...
export default function CurrentAccountPaymentFields({
    ...
    const calculateInstallmentDetails = () => {
        if (!moto) return { error: "Datos de la moto no disponibles.", schedule: [] as AmortizationScheduleEntry[] };
        ...
        if (pmtDenominator === 0 || (pmtNumerator === 0 && pmtDenominator === 0)) {
            ...
            return {
                ...
                totalInterest: 0,
                currency: moto.currency || "USD",
                warning: "No se pudo calcular la cuota con interés (división por cero), se usó cálculo simple.",
                schedule // Vacío o simple como el caso sin interés
            };
        }
        ...
        for (let i = 1; i <= installments; i++) {
            ...
            } else if (amortizationAmt > currentCapital) { // Por redondeos
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {details.schedule.map((item) => (
                                    <tr key={item.installmentNumber}>
                                        ...
                                        <td className="px-3 py-2 whitespace-nowrap text-right">{formatRoundedCurrency(item.capitalAtPeriodStart, details.currency || 'USD')}</td>
                                        <td className="px-3 py-2 whitespace-nowrap text-right">{formatRoundedCurrency(item.amortization, details.currency || 'USD')}</td>
                                        <td className="px-3 py-2 whitespace-nowrap text-right">{formatRoundedCurrency(item.interestForPeriod, details.currency || 'USD')}</td>
                                        <td className="px-3 py-2 whitespace-nowrap text-right font-semibold">{formatRoundedCurrency(item.calculatedInstallmentAmount, details.currency || 'USD')}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
    ...
    return (
        <div className="space-y-4 p-4 border rounded-md mt-4">
            ...
            <div>
                ...
                <Popover>
                    ...
                    <PopoverContent className="w-auto p-0">
                        <Calendar
                            mode="single"
                            selected={paymentData.currentAccountStartDate ? new Date(paymentData.currentAccountStartDate) : undefined}
            ...
            {(paymentData.currentAccountInstallments && paymentData.currentAccountInstallments > 0 && paymentData.annualInterestRate !== undefined) && (
