...
import { formatPrice, calculateRemainingAmount, calculateFinalPrice, getBestRatesByInstallment } from "./utils";
import { type PaymentFormData, type MotorcycleWithRelations } from "./types";
...
interface PaymentMethodStepProps {
    ...
    onPaymentDataChange: (e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement | HTMLTextAreaElement>) => void;
...
export default function PaymentMethodStep({
    ...
    const arePromotionsCompatible = (currentPromo: BankingPromotionDisplay, selectedPromos: BankingPromotionDisplay[]) => {
    ...
    const getGroupDisplayName = (groupValue: string, methodsInGroup: OrganizationPaymentMethodDisplay[]) => {
        if (groupValue === "tarjeta") return "Tarjeta";
        return methodsInGroup[0]?.card?.name || groupValue.charAt(0).toUpperCase() + groupValue.slice(1);
    };

    // Group payment methods (e.g., combine credit and debit)
    const groupedMethods = (organizationPaymentMethods || []).reduce((acc, orgMethod) => {
    ...
    return (
        <div className="space-y-6 mt-6">
            ...
            {!loadingOrgPaymentMethods && (!organizationPaymentMethods || organizationPaymentMethods.length === 0) && (
                ...
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-4">
                            <div> {/* Encapsulate Label and Select for main payment method */}
                            ...
                            {paymentData.metodoPago === 'tarjeta' && (
                                <div className="space-y-4 p-4 border rounded-md mt-4">
                                    ...
                                    <Input name="tarjetaNumero" placeholder="Número de Tarjeta" onChange={onPaymentDataChange} value={paymentData.tarjetaNumero || ""} />
                                    <Input name="tarjetaVencimiento" placeholder="MM/AA" onChange={onPaymentDataChange} value={paymentData.tarjetaVencimiento || ""} />
                                    <Input name="tarjetaCVV" placeholder="CVV" onChange={onPaymentDataChange} value={paymentData.tarjetaCVV || ""} />
                                    <Select name="tarjetaTipo" value={paymentData.tarjetaTipo || ""} onValueChange={(value) => onPaymentDataChange({ target: { name: "tarjetaTipo", value } } as React.ChangeEvent<HTMLSelectElement>)}>
                                    ...
                                    <Select name="banco" value={paymentData.banco || ""} onValueChange={(value) => onPaymentDataChange({ target: { name: "banco", value } } as React.ChangeEvent<HTMLSelectElement>)}>
                                        <SelectTrigger><SelectValue placeholder="Banco Emisor" /></SelectTrigger>
                                        <SelectContent>
                                    ...
                                    <Select name="cuotas" value={String(paymentData.cuotas || 1)} onValueChange={(value) => onPaymentDataChange({ target: { name: "cuotas", value } } as React.ChangeEvent<HTMLSelectElement>)}>
                                        <SelectTrigger>
                                        ...
                                        <SelectContent>
                                            ...
                                            {availableInstallmentPlans.map(plan => (
                                                <SelectItem key={plan.installments} value={String(plan.installments)}>
                                                    {plan.installments} cuotas {plan.interestRate === 0 ? 'sin interés' : `(${plan.interestRate}% interés)`}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                            ...
                            {paymentData.metodoPago === 'transferencia' && (
                                <div className="space-y-4 p-4 border rounded-md mt-4">
                                    ...
                                    <Input name="transferenciaCBU" placeholder="CBU/CVU/Alias" onChange={onPaymentDataChange} value={paymentData.transferenciaCBU || ""} />
                                    <Input name="transferenciaTitular" placeholder="Nombre del Titular" onChange={onPaymentDataChange} value={paymentData.transferenciaTitular || ""} />
                                    <Input name="transferenciaReferencia" placeholder="Referencia (Opcional)" onChange={onPaymentDataChange} value={paymentData.transferenciaReferencia || ""} />
                                </div>
                            )}
                            ...
                            {paymentData.metodoPago === 'cheque' && (
                                <div className="space-y-4 p-4 border rounded-md mt-4">
                                    ...
                                    <Input name="chequeNumero" placeholder="Número de Cheque" onChange={onPaymentDataChange} value={paymentData.chequeNumero || ""} />
                                    <Input name="chequeFecha" placeholder="Fecha de Emisión/Pago" type="text" onFocus={(e) => e.target.type = 'date'} onBlur={(e) => e.target.type = 'text'} onChange={onPaymentDataChange} value={paymentData.chequeFecha || ""} />
                                    <Input name="chequeEmisor" placeholder="Nombre del Emisor" onChange={onPaymentDataChange} value={paymentData.chequeEmisor || ""} />
                                    <Input name="chequeBanco" placeholder="Banco" onChange={onPaymentDataChange} value={paymentData.chequeBanco || ""} />
                                </div>
                            )}
                        ...
                        <div className="space-y-4">
                            <Card className="p-4 bg-secondary/50">
                                ...
                                <div className="space-y-1 text-sm">
                                    ...
                                    {paymentData.downPayment && paymentData.downPayment > 0 && paymentData.metodoPago === 'cuenta_corriente' && (
                                        <div className="flex justify-between text-green-600">
                                    ...
                                    {selectedPromotions.map(promo => (
                                        promo.discountRate ? (
                                            <div key={`discount-${promo.id}`} className="flex justify-between text-green-600">
                                                <span>Descuento ({promo.name}):</span>
                                                <span>- {formatPrice(getBasePrice() * (promo.discountRate / 100), moto?.currency)}</span>
                                            </div>
                                        ) : promo.surchargeRate ? (
                                            <div key={`surcharge-${promo.id}`} className="flex justify-between text-red-600">
                                                <span>Recargo ({promo.name}):</span>
                                                <span>+ {formatPrice(getBasePrice() * (promo.surchargeRate / 100), moto?.currency)}</span>
                                            </div>
                                        ) : null
                                    ))}
                                    <hr className="my-2" />
                                ...
                                {loadingPromotions ? (
                                    ...
                                    <div className="space-y-3">
                                        {applicablePromotions.map((promo) => {
                                            ...
                                            return (
                                                <div key={promo.id} className={`flex items-center space-x-3 p-3 border rounded-md ${isSelected ? 'border-primary bg-primary/10' : ''} ${isDisabled && !isSelected ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1">
                                                        <label htmlFor={`promo-${promo.id}`} className={`text-sm font-medium leading-none ${isDisabled && !isSelected ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                                                            {promo.name}
                                                        </label>
                                                        <p className="text-xs text-muted-foreground">
                                                            {promo.description || 'Descripción no disponible'}
                                                            {promo.discountRate && <Badge variant="secondary" className="ml-2">-{promo.discountRate}%</Badge>}
                                                            {promo.surchargeRate && <Badge variant="destructive" className="ml-2">+{promo.surchargeRate}%</Badge>}
                                                        </p>
                                                        {paymentData.metodoPago === 'tarjeta' && promo.installmentPlans && promo.installmentPlans.length > 0 && (
                                                            <div className="mt-1 flex flex-wrap gap-1">
                                                                {promo.installmentPlans.filter(p => p.isEnabled).map(plan => (
                                                                    <Badge key={plan.id} variant="outline" className="text-xs">
                                                                        {plan.installments}c {bestRatesMap[plan.installments]?.interestRate === plan.interestRate ? <Check className="h-3 w-3 ml-1 text-green-600" /> : null}
                                                                        {plan.interestRate > 0 ? `(${plan.interestRate}%)` : ' s/i'}
                                                                    </Badge>
                                                                ))}
                                                            </div>
                                                        )}
