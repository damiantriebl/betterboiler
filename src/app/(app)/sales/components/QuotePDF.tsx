import { Document, Page, Text, View, StyleSheet, Image as PDFImage, Image } from "@react-pdf/renderer";
import { type QuotePDFProps, type AmortizationScheduleEntry } from "@/types/quote";
...
export const QuotePDFDocument = ({
    ...
    const getPaymentMethodText = () => {
        switch (activeTab) {
            case 'efectivo': return 'Efectivo/Transferencia';
            case 'tarjeta': return `Tarjeta - ${paymentData.cuotas} cuota(s)`;
            case 'cuenta_corriente': return `Financiación - ${paymentData.currentAccountInstallments} cuotas (${paymentData.annualInterestRate}% interés)`;
    ...
    const formatAmount = typeof formatAmountProp === 'function'
        ? formatAmountProp
            ...
            <View style={styles.header}>
                ...
                <View style={styles.logoContainer}>
                    {organizationLogo
                        ...
                        : <Text>Logo</Text> // Fallback si no hay logo o no es base64
                    }
                </View>
            </View>
                ...
                <View style={styles.motorcycleInfo}>
                    <View style={styles.motorcycleDetails}>
                        <View style={styles.detailRow}>
                            ...
                            <Text style={styles.detailValue}>{motorcycle?.brand?.name} {motorcycle?.model?.name}</Text>
                        </View>
            ...
            <View style={styles.section}>
                ...
                {paymentData.discountValue > 0 && (
                    <View style={styles.detailRow}>
                        ...
                        <Text style={styles.detailValue}>
                            {paymentData.discountType === 'discount' ? '-' : '+'}{formatAmount(modifierAmount)}
                        </Text>
                    </View>
                )}
                <View style={styles.detailRow}>
                    <Text style={[styles.detailLabel, { fontWeight: 'bold' }]}>Precio Final:</Text>
                    <Text style={[styles.detailValue, { fontWeight: 'bold' }]}>{formatAmount(finalPrice)}</Text>
                </View>
                {activeTab === 'tarjeta' && paymentData.cuotas > 1 && (
                    <>
                        ...
                        <View style={styles.detailRow}>
                            ...
                            <Text style={styles.detailValue}>{formatAmount(finalPrice / paymentData.cuotas)}</Text>
                        </View>
                    </>
                )}
                {activeTab === 'cuenta_corriente' && (
                    <>
                        ...
                        <View style={styles.detailRow}>
                            <Text style={styles.detailLabel}>Valor de Cuota:</Text>
                            <Text style={styles.detailValue}>{formatAmount(installmentDetails.installmentAmount)}</Text>
                        </View>
                        <View style={styles.detailRow}>
                            <Text style={styles.detailLabel}>Total a Pagar:</Text>
                            <Text style={styles.detailValue}>{formatAmount(totalWithFinancing)}</Text>
                        </View>
                        {activeTab === 'cuenta_corriente' && 'totalInterest' in installmentDetails && installmentDetails.totalInterest !== undefined && installmentDetails.totalInterest > 0 && (
                            <View style={styles.detailRow}>
                                <Text style={styles.detailLabel}>Intereses Totales:</Text>
                                <Text style={styles.detailValue}>{formatAmount(installmentDetails.totalInterest)}</Text>
                            </View>
                        )}
                            ...
                            <View style={styles.table}>
                                ...
                                {installmentDetails.schedule.map((item) => (
                                    <View key={item.installmentNumber} style={styles.tableRow}>
                                        <Text style={[styles.tableCellLeft, { flex: 0.5 }]}>{item.installmentNumber}</Text>
                                        <Text style={styles.tableCellRight}>{formatAmount(item.capitalAtPeriodStart)}</Text>
                                        <Text style={styles.tableCellRight}>{formatAmount(item.amortization)}</Text>
                                        <Text style={styles.tableCellRight}>{formatAmount(item.interestForPeriod)}</Text>
                                        <Text style={styles.tableCellRight}>{formatAmount(item.calculatedInstallmentAmount)}</Text>
                                    </View>
                                ))}
                            </View>
                        )}
                            ...
                            <View style={styles.table}>
                                ...
                                {installmentDetails.schedule.map((detail: AmortizationScheduleEntry) => (
                                    <View key={detail.installmentNumber} style={styles.tableRow}>
                                        ...
                                        <Text style={styles.tableCellRight}>{formatAmount(detail.calculatedInstallmentAmount)}</Text>
                                        <Text style={styles.tableCellRight}>{formatAmount(detail.capitalAtPeriodEnd)}</Text>
                                    </View>
                                ))}
                            </View>
                            <View style={[styles.detailRow, styles.total, { justifyContent: 'flex-end' }]}>
                                <Text style={[styles.detailLabel, { textAlign: 'right', width: 'auto', marginRight: 5 }]}>Total con Financiación:</Text>
                                <Text style={[styles.detailValue, { textAlign: 'right', width: 'auto' }]}>{formatAmount(totalWithFinancing)}</Text>
                            </View>
                        </>
                    )}
                </>
