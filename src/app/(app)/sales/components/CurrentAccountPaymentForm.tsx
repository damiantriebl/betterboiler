...
import {
  ...
  baseCurrentAccountObject,
  currentAccountStatusSchema,
...
const calculateInstallment = (
  ...
  if (principal <= 0 || numberOfInstallments <= 0) {
    // Allow annualRate and periodsPerYear to be 0 for interest-free
  ...
  if (annualRate === 0) {
    // Handled above, but good for clarity
    return principal / numberOfInstallments;
  }
...
const formSchema = baseCurrentAccountObject
  .extend({
    annualInterestRate: z
      .number({ required_error: "La tasa de interés anual es requerida." })
      .min(0, "La tasa de interés no puede ser negativa."),
  })
  .refine(
    (data: z.infer<typeof baseCurrentAccountObject> & { annualInterestRate: number }) =>
      data.downPayment <= data.totalAmount,
    {
      // Keep the refinement for form validation
...
export function CurrentAccountPaymentForm({
  ...
  useEffect(() => {
    ...
    const finalInstallment = Number.parseFloat(installment.toFixed(2));
  ...
  const handleFormSubmit = async (data: CurrentAccountFormValues) => {
    ...
    const submissionData: CreateCurrentAccountInput = {
      ...
      installmentAmount: Number.parseFloat(data.installmentAmount.toFixed(2)),
  ...
  const isSubmitDisabled =
    ...
    Number.isNaN(calculatedInstallmentAmount);
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Complete los detalles para la financiación de la motocicleta. Precio total: $
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)}>
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              {errors.annualInterestRate && (
                <InputErrorMessage message={errors.annualInterestRate.message} />
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              {errors.numberOfInstallments && (
                <InputErrorMessage message={errors.numberOfInstallments.message} />
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {errors.paymentFrequency && (
                <InputErrorMessage message={errors.paymentFrequency.message} />
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Reminder Lead Time */}
            <div className="space-y-2">
              <Label htmlFor="reminderLeadTimeDays">
                Días de Aviso Previo al Vencimiento (Opcional)
              </Label>
              <Controller
              ...
              />
              {errors.reminderLeadTimeDays && (
                <InputErrorMessage message={errors.reminderLeadTimeDays.message} />
              )}
            </div>
          </div>
