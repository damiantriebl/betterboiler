...
export default function ManageGlobalBrands({
    initialGlobalBrands,
}: ManageGlobalBrandsProps) {
    ...
    const handleBrandUpdate = (formData: FormData) => {
        ...
        startBrandUpdateTransition(async () => {
            ...
            if (!result.success) {
                toast({ title: "Error al actualizar marca", description: result.error, variant: "destructive" });
                ...
                if (result.data) {
                    startBrandUpdateTransition(() => {
                        setBrands(prev => prev.map(b => b.id === result.data!.id ? { ...b, name: result.data!.name } : b));
                    });
                }
            }
        });
            ...
            if (!result.success) {
                ...
                if (result.data) {
                    ...
                    startModelActionTransition(() => {
                        setBrands(prev => prev.map(b => b.id === brandId ? { ...b, models: [...(b.models || []), newModel] } : b));
                    });
                }
    ...
    const handleUpdateModel = (formData: FormData) => {
        ...
        // Optimistic update
        startModelActionTransition(() => {
            setBrands(prev => prev.map(b => b.id === brandId ? { ...b, models: b.models?.map(m => m.id === modelId ? { ...m, name: newName } : m) } : b));
        });
        ...
        startModelActionTransition(async () => {
            ...
            if (!result.success) {
                toast({ title: "Error actualizar modelo", description: result.error, variant: "destructive" });
                ...
                if (result.data) {
                    ...
                    startModelActionTransition(() => {
                        setBrands(prev => prev.map(b => b.id === brandId ? { ...b, models: b.models?.map(m => m.id === updatedModel.id ? updatedModel : m) } : b));
                    });
                }
            }
        });
    ...
    const handleDeleteModel = (modelId: number, brandId: number) => {
        ...
        // Optimistic update
        startModelActionTransition(() => {
            setBrands(prev => prev.map(b => b.id === brandId ? { ...b, models: b.models?.filter(m => m.id !== modelId) } : b));
        });
        ...
        startModelActionTransition(async () => {
            ...
            if (!result.success) {
                toast({ title: "Error al eliminar modelo", description: result.error, variant: "destructive" });
