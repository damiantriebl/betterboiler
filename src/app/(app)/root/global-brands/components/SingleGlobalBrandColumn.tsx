...
import { GripVertical, Pencil, Plus, Save, Trash2, X, FileText, Image, FileSpreadsheet, MoreHorizontal } from 'lucide-react';
...
function SortableModelItem({
    ...
    const buildFileUrl = (fileType: 'image' | 'image_small' | 'spec_sheet' | 'other', fileName?: string): string => {
    ...
    const hasTechnicalSheet = !!(('specSheetUrl' in model && (model as any).specSheetUrl) || files.some((file: any) => file.type === 'spec_sheet' || (file.url && file.url.endsWith('.pdf'))));
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {isEditingBrandName ? (
                                <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <Button size="icon" variant="ghost" onClick={handleBrandNameSave} className="h-8 w-8">
                                        <Save className="h-4 w-4" />
                                    </Button>
                                    <Button size="icon" variant="ghost" onClick={handleBrandNameCancel} className="h-8 w-8">
                                        <X className="h-4 w-4" />
                                    </Button>
                                </div>
                            ) : (
