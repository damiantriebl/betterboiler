    ...
    if (mpError) {
      const errorMessages: Record<string, string> = {
        token_exchange_failed:
          "❌ Error intercambiando token. El código OAuth puede haber expirado o ya fue usado.",
        no_code_received: "❌ No se recibió código OAuth. El flujo fue cancelado o interrumpido.",
        no_session_or_organization:
          "❌ Sesión perdida. Por favor, inicia sesión nuevamente y vuelve a intentar.",
      ...
      const fullMessage = detail
        ? `${errorMessage}\n\nDetalle: ${decodeURIComponent(detail)}`
        : errorMessage;
