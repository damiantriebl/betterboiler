...
import { CalendarIcon, Plus, Percent, Trash, CreditCard, Building } from "lucide-react";
...
import { createBankingPromotion, updateBankingPromotion, getBankingPromotionDetails } from "@/actions/banking-promotions/manage-banking-promotions";
...
import { extractBankAndCardFromBankCardId, findBankCardId, groupBankCardsByBank } from "@/utils/banking-utils";
...
export default function PromotionForm({
    ...
    const initialIsDiscount = promotion ? (promotion.discountRate !== undefined && promotion.discountRate !== null && promotion.discountRate > 0) : true;
    const initialRateValue = promotion
        ? (initialIsDiscount
            ? (promotion.discountRate !== null && promotion.discountRate !== undefined ? promotion.discountRate : undefined)
            : (promotion.surchargeRate !== null && promotion.surchargeRate !== undefined ? promotion.surchargeRate : undefined))
        : undefined;
    ...
    const form = useForm<FormValues>({
        ...
        defaultValues: {
            ...
            installmentPlans: promotion?.installmentPlans?.map(plan => ({
                id: String(plan.id),
    ...
    const isCreditCard = !!selectedPaymentMethod && paymentMethods.some(method =>
        method.id.toString() === selectedPaymentMethod && method.type === "credit"
    );
    ...
    useEffect(() => {
        const bankIdNum = watchBankId ? parseInt(watchBankId) : null;
        const cardIdNum = watchCardId ? parseInt(watchCardId) : null;
    ...
    useEffect(() => {
        if (promotion) {
            ...
            const { bankId: initBankId, cardId: initCardId } = extractBankAndCardFromBankCardId(bankCards, promotion.cardId);
            form.reset({
                name: promotion.name,
                description: promotion.description ?? "",
    ...
    const onSubmit = (_data: FormValues) => {
        ...
        const installmentPlans = form.getValues("installmentPlans").map(plan => ({
            id: typeof plan.id === 'string' && plan.id.startsWith('temp-') ? undefined : parseInt(plan.id),
            installments: plan.installments,
            interestRate: plan.interestRate,
            isEnabled: plan.isEnabled
        }));
            ...
            try {
                ...
                if (!isEditing) {
                    ...
                    const result = await createBankingPromotion({
                        ...
                        bankId: values.bankId ? parseInt(values.bankId) : null,
                        cardId: values.cardId ? parseInt(values.cardId) : null,
                        discountRate,
                        surchargeRate,
                    ...
                    if (result.success && result.data) {
                        ...
                        const selectedMethod = paymentMethods.find(m => m.id.toString() === values.paymentMethodId);

                        // Obtener banco y tarjeta si están seleccionados
                        const createdBankId = values.bankId ? parseInt(values.bankId) : null;
                        const createdCardId = values.cardId ? parseInt(values.cardId) : null;

                        const bank: Bank | null = createdBankId ? { id: createdBankId, name: banksWithCards.find(b => b.bank.id === createdBankId)?.bank.name || "" } : null;
                ...
                // For updating existing promotion
                    ...
                    if (currentPromoDetails.success && currentPromoDetails.data) {
                        ...
                        console.log("Updating with discountRate:", discountRate, "surchargeRate:", surchargeRate);
                    }
                    ...
                    const result = await updateBankingPromotion({
                        ...
                        paymentMethodId: parseInt(_data.paymentMethodId),
                        bankId: _data.bankId ? parseInt(_data.bankId) : null,
                        cardId: _data.cardId ? parseInt(_data.cardId) : null,
                        discountRate,
                        surchargeRate,
                    ...
                    if (result.success) {
                        ...
                        // Obtener el método de pago seleccionado
                        const selectedMethod = paymentMethods.find(m => m.id.toString() === _data.paymentMethodId);

                        // Obtener banco y tarjeta si están seleccionados
                        const updatedBankId = _data.bankId ? parseInt(_data.bankId) : null;
                        const updatedCardId = _data.cardId ? parseInt(_data.cardId) : null;

                        const bank: Bank | null = updatedBankId ? { id: updatedBankId, name: banksWithCards.find(b => b.bank.id === updatedBankId)?.bank.name || "" } : null;
                        ...
                        const updatedInstallmentPlans = _data.installmentPlans.map(plan => {
                            ...
                            return {
                                id: pid.startsWith('temp-') ? -(Date.now()) : parseInt(pid),
                ...
                const errorMessage = error?.issues?.[0]?.message || error.message || "Ocurrió un error al guardar la promoción.";
                toast({
                    title: "Error",
                                                ...
                                                <FormItem>
                                                    ...
                                                    <FormControl>
                                                        <BankCardSelector
                                                            ...
                                                            selectedBankId={field.value ? parseInt(field.value) : null}
                                                            selectedCardId={form.watch("cardId") ? parseInt(form.watch("cardId")!) : null}
                                                            onSelectBank={(bankId) => {
                                                    ...
                                                    <FormDescription>
                                                        Selecciona un banco y una tarjeta específica o "Todos los bancos" para aplicar a cualquiera.
                                                    </FormDescription>
                                                    <FormMessage />
                                                </FormItem>
                                            )}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="flex flex-col">
                                                ...
                                                <FormDescription>
                                                    Opcional. Cuándo comienza la promoción.
                                                </FormDescription>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem className="flex flex-col">
                                                ...
                                                <FormDescription>
                                                    Opcional. Cuándo termina la promoción.
                                                </FormDescription>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormDescription>
                                                Días de la semana en que aplica esta promoción. Si no se selecciona ninguno, aplica todos los días.
                                            </FormDescription>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                ...
                                <div className="space-y-4">
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel>
                                                    {watchIsDiscount ? "Porcentaje de descuento (%)" : "Porcentaje de recargo (%)"}
                                                </FormLabel>
                                                <FormControl>
                                                    <Input
                                                        type="number"
                                                        placeholder={watchIsDiscount ?
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormDescription>
                                                    Monto máximo para aplicar la promoción.
                                                </FormDescription>
                                                <FormMessage />
                                            </FormItem>
                                        )}
