...
import type { BankingPromotionDisplay, PromotionCalculation } from "@/types/banking-promotions";
...
export default function PromotionsCalculator({ promotions }: PromotionsCalculatorProps) {
  ...
  const selectedPromotion = promotions.find((p) => p.id === Number(selectedPromotionId));

  // Get available installments from the selected promotion
  const availableInstallments =
    selectedPromotion?.installmentPlans
  ...
  return (
    <Card>
      ...
      <CardContent>
          ...
          <div className="space-y-2">
            ...
            >
              <SelectTrigger id="installments">
                <SelectValue
                  placeholder={
                    availableInstallments.length ? "Seleccione cuotas" : "No hay cuotas disponibles"
                  }
                />
              </SelectTrigger>
              <SelectContent>
        ...
        {/* Calculate button */}
        <div className="mt-4">
          <Button onClick={handleCalculate} disabled={!selectedPromotionId || isPending}>
            {isPending ? (
