...
import { Wallet, Plus, PercentIcon, Trash2, ToggleLeft, ToggleRight } from "lucide-react";
...
import { toggleBankingPromotion, deleteBankingPromotion } from "@/actions/banking-promotions/manage-banking-promotions";
...
export default function ManageBankingPromotions({
    ...
    const [optimisticPromotions, updateOptimisticPromotions] = useOptimistic(
        promotions,
        (state, update: BankingPromotionDisplay[] | ((prev: BankingPromotionDisplay[]) => BankingPromotionDisplay[])) => {
    ...
    const [editingPromotion, setEditingPromotion] = useState<BankingPromotionDisplay | undefined>(undefined);
    ...
    const formatPromotionConditions = (promotion: BankingPromotionDisplay) => {
        ...
        if (promotion.discountRate && typeof promotion.discountRate === 'number' && promotion.discountRate > 0) {
            conditions.push(`${promotion.discountRate}% de descuento`);
        }

        // Check if surcharge rate is present and greater than zero
        if (promotion.surchargeRate && typeof promotion.surchargeRate === 'number' && promotion.surchargeRate > 0) {
        ...
        if (installments.length > 0) {
            ...
            if (noInterestPlans.length > 0) {
                conditions.push(`${noInterestPlans.map(p => p.installments).join(", ")} cuotas sin interés`);
            }

            const interestPlans = installments.filter(p => p.interestRate > 0);
            if (interestPlans.length > 0) {
                conditions.push(`${interestPlans.map(p => `${p.installments}x (${p.interestRate}%)`).join(", ")} con interés`);
            }
        }
                                        ...
                                        <Card key={promotion.id} className={promotion.isEnabled ? "" : "opacity-60"}>
                                                ...
                                                <div className="flex justify-between items-start">
                                                    ...
                                                    <div className="flex space-x-1">
                                                        <Toggle
                                                            ...
                                                            onPressedChange={() => handleTogglePromotion(promotion.id, promotion.isEnabled)}
                                                            disabled={isPending}
                                                        >
                                                            {promotion.isEnabled ? (
                                            ...
                                            <CardContent className="pt-0">
                                                <div className="text-xs text-muted-foreground grid grid-cols-2 gap-2">
                                                    {promotion.minAmount && (
                                                        <div>
                                                            <span className="font-medium">Mín:</span> {formatCurrency(promotion.minAmount)}
                                                        </div>
                                                    )}

                                                    {promotion.maxAmount && (
                                                        <div>
                                                            <span className="font-medium">Máx:</span> {formatCurrency(promotion.maxAmount)}
                                                        </div>
                                                    )}

                                                    {promotion.startDate && (
                                                        <div>
                                                            <span className="font-medium">Desde:</span> {formatDate(new Date(promotion.startDate))}
                                                        </div>
                                                    )}

                                                    {promotion.endDate && (
                                                        <div>
                                                            <span className="font-medium">Hasta:</span> {formatDate(new Date(promotion.endDate))}
                                                        </div>
                                                    )}
                                                </div>
                                            </CardContent>
