...
export default function ManageBrands({ initialOrganizationBrands, organizationId }: ManageBrandsProps) {
    const [associations, setAssociations] = useState<OrganizationBrandDisplayData[]>(initialOrganizationBrands);
    ...
    const handleAssociationDragEnd = (event: DragEndEvent) => {
        ...
        const associationsWithUpdatedOrder = newlyOrderedAssociations.map((item, index) => ({ ...item, order: index }));
    ...
    const handleAssociationDelete = (organizationBrandIdToDelete: number) => {
        ...
        startDissociateTransition(async () => {
            ...
            if (!result.success) {
                toast({ title: "Error al desasociar Marca", description: result.error, variant: "destructive" });
                setAssociations(previousAssociations);
            } else {
                toast({ title: "Marca desasociada", description: `Marca "${assocToDelete?.brand.name ?? 'Desconocida'}" desasociada.` });
            }
        });
    };
    ...
    const handleAssociationUpdate = (formData: FormData) => {
        ...
        startAssociationUpdateTransition(async () => {
            ...
            if (!result.success) {
                toast({ title: "Error al actualizar Color Asociación", description: result.error, variant: "destructive" });
            } else {
    ...
    const openAddModal = () => {
        if (!organizationId) {
            toast({ title: "Error", description: "Se requiere una organización para añadir marcas.", variant: "destructive" });
            return;
        }
        setIsModalOpen(true);
    };
    ...
    const handleAddModel = (formData: FormData) => {
        ...
        startModelActionTransition(async () => {
            ...
            if (!result.success) {
                toast({ title: "Error al añadir Modelo", description: result.error, variant: "destructive" });
            } else {
    ...
    const handleUpdateModel = (formData: FormData) => {
        ...
        startModelActionTransition(async () => {
            ...
            if (!result.success) {
                toast({ title: "Error al reemplazar Modelo", description: result.error, variant: "destructive" });
            } else {
    ...
    const handleModelsOrderUpdate = (brandId: number, orderedModels: { modelId: number; order: number }[]) => {
        ...
        const updatedModels = orderedModels.map((modelOrder) => {
            const originalModel = associations[brandIndex].brand.models.find(m => m.id === modelOrder.modelId);
        ...
        startModelActionTransition(async () => {
            const result = await updateOrganizationModelsOrder(null, { modelOrders: orderedModels, brandId });
            if (!result.success) {
                toast({ title: "Error al guardar orden Modelos", description: result.error, variant: "destructive" });
                setAssociations(previousAssociations);
            } else {
        ...
        <Card>
            ...
            <CardContent>
                <p className="text-sm text-muted-foreground mb-4">
                    Arrastra las marcas asociadas para cambiar su orden. Edita colores y modelos dentro de cada columna.
                </p>
                {!organizationId ? (
                ...
                {organizationId && associations.length === 0 && (
                    <p className="text-center text-muted-foreground mt-4">No hay marcas asociadas a esta organización.</p>
                )}
            </CardContent>
