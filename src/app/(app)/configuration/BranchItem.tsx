...
export default function SucursalItem({
    ...
    return (
        <Card ref={setNodeRef} style={style} className={cn("relative", isDragging ? "shadow-lg" : "shadow-sm")}>
            <CardHeader className="p-3 flex flex-row items-center justify-between space-x-2">
                ...
                <div className="flex items-center space-x-1 flex-shrink-0">
                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={handleEditClick} aria-label={`Editar sucursal ${name}`} disabled={isUpdating || isEditing}>
                        <Edit className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={handleDeleteClick} aria-label={`Eliminar sucursal ${name}`} disabled={isUpdating || isEditing}>
