            ...
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                <p className="mt-2 font-medium">
                  Nota: Si esta asociación está siendo utilizada en alguna promoción, no podrá ser
                  eliminada.
                </p>
              </AlertDialogDescription>
            </AlertDialogHeader>
...
export default function ManageBankCards({
  ...
  // Real state
  const [actualBankCardsState, setActualBankCardsState] =
    useState<BankWithCards[]>(initialBanksWithCards);
  // Optimistic state
  ...
  const handleDissociateCard = (bankCardId: number, bankId: number) => {
    // Optimistic Update Calculation
    const cardName =
      actualBankCardsState.find((b) => b.bank.id === bankId)?.cards.find((c) => c.id === bankCardId)
        ?.cardType?.name || "la tarjeta";
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        toast({
          ...
          description: `Se eliminó la asociación con ${cardName}.`,
        });
      } else {
        toast({
          title: "Error",
  ...
  const handleToggleCardStatus = (bankCardId: number, isEnabled: boolean, bankId: number) => {
    ...
    const newState = actualBankCardsState.map((bwc) => {
      if (bwc.bank.id === bankId) {
        return {
          ...bwc,
          cards: bwc.cards.map((card) => (card.id === bankCardId ? { ...card, isEnabled } : card)),
        };
      }
      return bwc;
    });
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        toast({
          title: isEnabled ? "Habilitada" : "Deshabilitada",
          description: `Se ${isEnabled ? "habilitó" : "deshabilitó"} ${cardName}.`,
        });
      } else {
        toast({
          title: "Error",
    ...
    startTransition(async () => {
      ...
      try {
        ...
        toast({
          title: "Error",
          description:
            "Ocurrió un error al actualizar el orden. Algunas tarjetas podrían tener un ID temporal.",
          variant: "destructive",
        });
      }
    });
  ...
  const handleAddBank = (bankId: number) => {
    ...
    if (!bankToAdd) {
      toast({
        title: "Error",
        description: "No se pudo encontrar el banco seleccionado.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleCardTypeCreated = (newCardType: CardType) => {
    ...
    toast({
      title: "Tipo de tarjeta creado",
      description: `'${newCardType.name}' se ha creado. Estará disponible la próxima vez que cargue esta página.`,
    });
  };
  ...
  const currentBankAssociatedCardTypeIds = new Set(
    optimisticBankCardsState
      .find((b) => b.bank.id === associatingBank?.bank.id)
