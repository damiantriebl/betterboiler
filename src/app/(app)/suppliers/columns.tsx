...
export const columns: ColumnDef<Proveedor>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => row.original.contactoNombre || <span className="text-muted-foreground italic">N/A</span>,
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return <div className="truncate max-w-xs">{domicilio || <span className="text-muted-foreground italic">N/A</span>}</div>
        },
    },
    {
        accessorKey: "estado",
        ...
        cell: ({ row }) => {
            ...
            return <span className={`px-2 py-1 rounded-full text-xs font-semibold ${estado === 'activo' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
