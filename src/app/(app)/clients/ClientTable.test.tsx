...
describe('ClientTable', () => {
    ...
    it('ordena por nombre al hacer click en el botón de Nombre', async () => {
        ...
        const nombreCeldas = screen.getAllByRole('cell').filter(cell =>
            cell.className.includes('font-medium')
        );
        expect(nombreCeldas[0]).toHaveTextContent('John Doe');
        expect(nombreCeldas[1]).toHaveTextContent('Jane Smith');

        await user.click(nombreBtn);
        const nombreCeldasDesc = screen.getAllByRole('cell').filter(cell =>
            cell.className.includes('font-medium')
        );
        expect(nombreCeldasDesc[0]).toHaveTextContent('Jane Smith');
        expect(nombreCeldasDesc[1]).toHaveTextContent('John Doe');
    });
    ...
    it('llama a onDelete cuando se hace click en Eliminar y se confirma', async () => {
        ...
        }, { timeout: 3000 }); // Aumentar el timeout para dar más tiempo
    });
});