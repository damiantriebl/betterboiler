...
export function BatchPreview({ formData, availableBrands, availableColors, availableBranches, suppliers }: BatchPreviewProps) {
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 items-start sticky top-20">
            ...
            <Card className="flex-1 w-full">
                ...
                <CardContent>
                    <div className="max-h-[450px] overflow-y-auto border rounded-md"> {/* Contenedor scrollable con borde */}
                        <Table>
                            <TableHeader className="sticky top-0 bg-background z-10"> {/* Header fijo */}
                                <TableRow>
                                    <TableHead className="w-[40px]">#</TableHead> {/* Ajustar ancho si es necesario */}
                                    ...
                                    formData.units.map((unit, index) => {
                                        ...
                                        return (
                                            <TableRow key={unit.idTemporal || index}>
                                                    ...
                                                    {selectedColor ? (
                                                        <TooltipProvider delayDuration={100}>
                                                            <Tooltip>
                                                                ...
                                                                <TooltipContent><p>{selectedColor.name}</p></TooltipContent>
                                                            </Tooltip>
                                                        </TooltipProvider>
                                                    ) : (
                                                        renderValue(null)
                                                ...
                                                <TableCell>{renderValue(unit.state, 'STOCK')}</TableCell> {/* Mostrar Estado */}
