...
export function MotoPreviewCard({ formData, availableBrands, availableColors }: MotoPreviewCardProps) {
    ...
    const getDisplayValue = (value: string | number | null | undefined, placeholder: string = 'N/A') => {
    ...
    return (
        <Card className={cn(
            ...
            <CardHeader className="pb-2 pt-4"> {/* Menos padding vertical */}
                <CardTitle className="text-lg">Vista Previa</CardTitle> {/* Título más pequeño */}
            </CardHeader>
            <CardContent className="space-y-2 text-sm pb-4"> {/* Menos espacio y padding */}
                ...
                <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium">{formData.cilindrada ? `${formData.cilindrada} cc` : '---'}</span>
                </div>
                <div className="flex justify-between items-center">
