...
import {
    createPettyCashWithdrawalSchema,
...
interface WithdrawFormProps {
    ...
    onSubmitAction: (prevState: CreatePettyCashWithdrawalState, formData: FormData) => Promise<CreatePettyCashWithdrawalState>;
    onClose: () => void;
    users: User[];
...
const WithdrawForm = ({ depositId, onSubmitAction, onClose, users, organizationId }: WithdrawFormProps) => {
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-4 pt-4">
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <Select onValueChange={field.onChange} defaultValue={field.value} disabled={users.length === 0}>
                                        <FormControl>
                                            <SelectTrigger>
                                                <SelectValue placeholder={users.length === 0 ? "No hay usuarios disponibles" : "Seleccionar beneficiario"} />
                                            </SelectTrigger>
                                        </FormControl>
                                        <SelectContent>
                                            {users.map(user => (
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input placeholder="Ej: Compra de suministros, viáticos" {...field} value={field.value ?? ""} />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        ...
                        <DialogFooter className="pt-6">
                            <DialogClose asChild>
                                <Button type="button" variant="outline" onClick={() => { form.reset(); onClose(); }} aria-label="Cancelar">
                                    Cancelar
                                </Button>
                            </DialogClose>
                            <Button type="submit" disabled={isPending || !form.formState.isValid} aria-label="Confirmar Retiro">
                                {isPending ? (
