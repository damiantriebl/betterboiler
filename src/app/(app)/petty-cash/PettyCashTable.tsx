    ...
    if (!deposits || deposits.length === 0) {
        return (
            <div className="mt-6 border rounded-lg p-8 text-center">
                ...
                <h3 className="mt-2 text-lg font-medium text-gray-700 dark:text-gray-200">Sin Depósitos de Caja Chica</h3>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Aún no se han registrado depósitos.</p>
...
const DepositEntry = ({ deposit, users, onAddWithdrawal, onAddSpend, onDeleteSpend, onDeleteWithdrawal, onDeleteDeposit }: DepositEntryProps) => {
    ...
    const getDepositStatusBadge = (status: PettyCashDepositStatus) => {
        switch (status) {
            case "OPEN": return <Badge className="bg-blue-100 text-blue-700">Abierto</Badge>;
            case "CLOSED": return <Badge variant="destructive" className="bg-gray-100 text-gray-700">Cerrado</Badge>;
            case "PENDING_FUNDING": return <Badge className="bg-yellow-100 text-yellow-700">Pendiente Fondos</Badge>;
        ...
        <>
            <TableRow className="bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                <TableCell className="px-2 py-2">
                    ...
                    >
                        {isDepositOpen ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
...
const WithdrawalEntry = ({ withdrawal, users, onAddSpend, onDeleteSpend, onDeleteWithdrawal }: WithdrawalEntryProps) => {
    ...
    const getWithdrawalStatusBadge = (status: PettyCashWithdrawalStatus) => {
        switch (status) {
            case "PENDING_JUSTIFICATION": return <Badge className="bg-amber-100 text-amber-700">Pend. Justif.</Badge>;
            case "PARTIALLY_JUSTIFIED": return <Badge className="bg-sky-100 text-sky-700">Parc. Justif.</Badge>;
            case "JUSTIFIED": return <Badge className="bg-emerald-100 text-emerald-700">Justificado</Badge>;
            case "NOT_CLOSED": return <Badge className="bg-rose-100 text-rose-700">No Cerrado</Badge>;
            default: return <Badge variant="secondary">{status}</Badge>;
        }
    };
        ...
        <>
            <TableRow className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors text-sm">
                <TableCell className="px-1 py-1">
                    ...
                    >
                        {isWithdrawalOpen ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
            ...
            {isWithdrawalOpen && (
                <TableRow className="bg-white dark:bg-gray-850">
                    <TableCell colSpan={8} className="p-0"> {/* 8 columns for withdrawal's content */}
...
const SpendEntry = ({ spend, onDelete }: SpendEntryProps) => {
    return (
        <TableRow className="text-xs hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
            ...
            <TableCell>
                {spend.ticketUrl ? (
                    <a href={spend.ticketUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-700 hover:underline flex items-center">
