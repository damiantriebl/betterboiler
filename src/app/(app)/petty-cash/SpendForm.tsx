...
export default function SpendForm({ withdrawalId, onClose, organizationId }: SpendFormProps) {
  ...
  useEffect(() => {
    ...
    if (state.status === "error") {
      toast({
        ...
        description:
          state.errors?._form?.join(", ") || state.message || "Ocurrió un error desconocido.",
      });
      if (state.errors) {
          ...
          if (messages.length > 0) {
            if (fieldKey === "ticketFile") {
              form.setError("root.serverError", {
                type: "server",
                message: `Archivo: ${messages.join(", ")}`,
              });
            } else if (form.control._fields && fieldKey in form.control._fields) {
              form.setError(fieldKey as keyof SpendFormData, {
                type: "server",
  ...
  const attemptFormSubmission = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (currentWithdrawalBalance !== null && spendAmount > currentWithdrawalBalance) {
      form.setError("amount", {
        type: "manual",
        message: `El monto no puede exceder el saldo disponible del retiro: ${currentWithdrawalBalance}`,
      });
      return;
    }
  };
  ...
  const handleSubmit = async (values: UpdatePettyCashMovementInput) => {
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-md">
        ...
        <Form {...form}>
          <form ref={formRef} onSubmit={attemptFormSubmission} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    Descripción del Gasto {watchedMotive === "otros" ? "(Requerido)" : "(Opcional)"}
                  </FormLabel>
                  <FormControl>
                    <Textarea
                      placeholder={
                        watchedMotive === "otros"
                          ? "Detalle el gasto..."
                          : "Ej: Compra de insumos de oficina (opcional)"
                      }
                      {...field}
                      value={field.value ?? ""}
                    />
            ...
            <FormItem>
              ...
              {ticketErrorMessage && (
                <p className="text-sm font-medium text-destructive">{ticketErrorMessage}</p>
              )}
            </FormItem>
