...
if (process.env.NEXT_PUBLIC_MP_PUBLIC_KEY) {
    ...
    console.warn("NEXT_PUBLIC_MP_PUBLIC_KEY no está definida. El componente de pago de Mercado Pago podría no funcionar.");
}
    ...
    return (
        <div>
            ...
            {paymentResult && !isProcessing && (
                <div>
                    <p>Resultado: {paymentResult.status || 'N/A'} - ID Transacción (si aplica): {paymentResult.id || 'N/A'}</p>
                    {paymentResult.error && <p className="error">Error del backend: {paymentResult.error} {paymentResult.details ? JSON.stringify(paymentResult.details) : ''}</p>}
                </div>
            )}
