...
export default function ProfileForm({
    user,
}: { user: { id: string; name: string; email: string; phone?: string | null; address?: string | null, profileOriginal?: string | null, profileCrop?: string | null } }) {
    ...
    const handleImageUploadAndCrop = async ({ originalFile, croppedFile }: UploadResult) => {
        ...
        try {
            ...
            const resOriginal = await fetch(`/api/s3/get-signed-url?name=${encodeURIComponent(originalFileName)}&operation=put&contentType=${encodeURIComponent(originalFile.type)}`);
            const dataOriginal = await resOriginal.json();
            if (!dataOriginal.success?.url) throw new Error("No se pudo obtener la URL firmada para la imagen original");
            await fetch(dataOriginal.success.url, { method: "PUT", body: originalFile, headers: { "Content-Type": originalFile.type } });
            ...
            if (croppedFile) {
                ...
                const resCrop = await fetch(`/api/s3/get-signed-url?name=${encodeURIComponent(cropFileName)}&operation=put&contentType=${encodeURIComponent(croppedFile.type)}`);
                const dataCrop = await resCrop.json();
                if (!dataCrop.success?.url) throw new Error("No se pudo obtener la URL firmada para la imagen recortada");
                await fetch(dataCrop.success.url, { method: "PUT", body: croppedFile, headers: { "Content-Type": croppedFile.type } });
            ...
            toast({ title: "Imagen procesada", description: "La imagen de perfil está lista para guardar." });
        } catch (err) {
            console.error("Error al subir imagen de perfil:", err);
            toast({ title: "Error de subida", description: (err as Error).message || "No se pudo subir la imagen de perfil", variant: "destructive" });
            ...
            <CardContent>
                <Form {...form}>
                    <form onSubmit={onSubmit} className="space-y-6">
                        ...
                        <FormField control={form.control} name="profileOriginalKey" render={({ field }) => <input type="hidden" {...field} />} />
                        <FormField control={form.control} name="profileCropKey" render={({ field }) => <input type="hidden" {...field} />} />
                        ...
                        <div className="space-y-2">
                                    ...
                                    <div className="w-32 h-32 rounded-full border overflow-hidden bg-muted flex items-center justify-center">
                                        {isLoadingPreview ? (
                                            ...
                                            <Image
                                                ...
                                                onError={() => {
                                                    if (profileImagePreview && !profileImagePreview.startsWith('blob:') && !profileImagePreview.startsWith('http')) {
                                                    } else if (profileImagePreview) {
                                                        const fallbackKey = form.getValues("profileCropKey") || form.getValues("profileOriginalKey");
                            ...
                            <UploadCropperButton
                                placeholder={profileImagePreview ? "Cambiar imagen de perfil" : "Subir imagen de perfil"}
                            ...
                            />
                            {isUploadingImage && <span className="text-xs text-muted-foreground">Procesando imagen...</span>}
                        </div>
