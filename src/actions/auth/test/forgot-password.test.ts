...
describe("forgotPasswordAction", () => {
  ...
  describe("🔧 Auth Client Errors", () => {
    it("should handle USER_NOT_FOUND error", async () => {
      ...
      expect(result.error).toBe(
        "Usuario no encontrado. No existe ninguna cuenta registrada con este email.",
      );
    });
    ...
    it("should handle network errors", async () => {
      ...
      // Act & Assert
      await expect(
        forgotPasswordAction(initialState, createFormData("test@example.com")),
  ...
  describe("🔒 Security Considerations", () => {
    it("should not leak information about user existence", async () => {
      ...
      // User exists case
      mockAuthClient.forgetPassword.mockResolvedValue({ error: null });
      const result1 = await forgotPasswordAction(
        initialState,
        createFormData("existing@example.com"),
      );

      // User doesn't exist case
      mockAuthClient.forgetPassword.mockResolvedValue({ error: null });
      const result2 = await forgotPasswordAction(
        initialState,
        createFormData("nonexistent@example.com"),
      );
  ...
  describe("🚀 Edge Cases", () => {
    it("should handle very long email addresses", async () => {
      // Arrange
      const longEmailPrefix = "a".repeat(100);
      const longEmail = `${longEmailPrefix}@example.com`;
      mockSchema.safeParse.mockReturnValue({
        success: true,
