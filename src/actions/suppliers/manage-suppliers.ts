...
export async function createSupplier(formData: SupplierFormData): Promise<SupplierActionResult> {
    ...
    try {
        ...
        if (existingSupplier) {
            return { success: false, error: `Ya existe un proveedor con CUIT ${dataToSave.taxIdentification} en esta organización.` };
        }
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
            if (error.code === 'P2002') {
                 return { success: false, error: `Error de base de datos: Ya existe un proveedor con datos únicos conflictivos.` };
            }
        }
...
export async function deleteSupplier(supplierId: number): Promise<Omit<SupplierActionResult, 'supplier'>> {
    const session = await auth.api.getSession({ headers: await headers() });
    if (!session?.user?.organizationId) {
        return { success: false, error: "Usuario no autenticado o sin organización." };
    }
    ...
    try {
        ...
        if (!supplier || supplier.organizationId !== organizationId) {
            return { success: false, error: "Proveedor no encontrado o no pertenece a esta organización." };
        }
        ...
        console.error("Error eliminando proveedor:", error);
         if (error instanceof Prisma.PrismaClientKnownRequestError) {
             if (error.code === 'P2003') {
                 return { success: false, error: "No se puede eliminar el proveedor porque tiene registros asociados (ej: motos)." };
             }
        }
    ...
    try {
        ...
        if (dataToUpdate.taxIdentification !== existingSupplier.taxIdentification) {
            ...
            if (conflictingSupplier) {
                return { success: false, error: `Ya existe otro proveedor con CUIT ${dataToUpdate.taxIdentification} en esta organización.` };
            }
        }
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError) {
            if (error.code === 'P2002') {
                 return { success: false, error: `Error de base de datos: Datos únicos conflictivos al actualizar.` };
            }
             if (error.code === 'P2025') { // Record to update not found
                 return { success: false, error: "Error de base de datos: Proveedor no encontrado para actualizar." };
            }
        }
        return { success: false, error: "No se pudo actualizar el proveedor debido a un error interno." };
    }
}
