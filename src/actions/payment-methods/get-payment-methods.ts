...
export async function getOrganizationPaymentMethods(organizationId: string): Promise<OrganizationPaymentMethodDisplay[]> {
  try {
    ...
    const formattedMethods: OrganizationPaymentMethodDisplay[] = organizationMethods.map((orgMethod: any) => ({
      id: orgMethod.id,
