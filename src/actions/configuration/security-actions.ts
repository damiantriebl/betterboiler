...
export async function toggleSecureMode(isEnabled: boolean): Promise<ToggleSecureModeResult> {
  const session = await getOrganizationIdFromSession();
  if (
    ...
    return {
      ...
      error:
        session.error ||
        "Usuario no autenticado o falta información de sesión (email, orgId, role).",
    };
  }
  ...
  try {
    ...
    console.log(
      `[toggleSecureMode] Usando para QR: Issuer=${OTP_ISSUER}, Algorithm=${OTP_ALGORITHM}, Digits=${OTP_DIGITS}, Period=${OTP_PERIOD}`,
    );
    ...
    const dataToUpdate: Partial<typeof currentOrg> & {
      secureModeEnabled: boolean;
      otpVerified: boolean;
...
export async function verifyOtpSetup(tokenAsNumber: number): Promise<VerifyOtpSetupResult> {
  ...
  if (!session.organizationId || !session.userEmail || session.error) {
    return {
      success: false,
      error:
        session.error || "Usuario no autenticado o falta información de sesión (email, orgId).",
    };
  }
  const { organizationId, userEmail } = session;
  ...
  console.log(
    `[verifyOtpSetup] OTP_DIGITS_FROM_CONFIG: ${OTP_DIGITS_FROM_CONFIG} (Tipo: ${typeof OTP_DIGITS_FROM_CONFIG})`,
  );
  ...
  if (Number.isNaN(OTP_DIGITS_FROM_CONFIG) || OTP_DIGITS_FROM_CONFIG <= 0) {
    console.error(
      `[verifyOtpSetup] ERROR CRÍTICO: OTP_DIGITS_FROM_CONFIG no es un número válido (${OTP_DIGITS_FROM_CONFIG}). No se puede validar el token.`,
    );
    return {
      success: false,
      error: "Error interno en la configuración de OTP. Contacte al administrador.",
    };
  }
  ...
  try {
    ...
    if (!organization.secureModeEnabled) {
      return {
        success: false,
        error: "El modo seguro no está activado. No se puede verificar OTP.",
      };
    }
