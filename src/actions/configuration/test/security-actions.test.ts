...
import { getSecuritySettings, toggleSecureMode, verifyOtpSetup } from "../security-actions";
...
vi.mock("otpauth", () => {
  ...
  const MockTOTP = vi.fn(() => ({
    toString: () =>
      "otpauth://totp/test@example.com?secret=MOCKED_SECRET&issuer=Test%20Apex%20Software",
  ...
  describe("🔒 toggleSecureMode", () => {
    ...
    describe("❌ Error Handling", () => {
      ...
      it("should handle database errors", async () => {
        ...
        expect(mockConsole.error).toHaveBeenCalledWith("Error toggling secure mode:", dbError);
      });
    ...
    describe("🎯 Environment Configuration", () => {
      it("should use default values when environment variables are missing", async () => {
        // Arrange
        process.env.OTP_ISSUER = undefined;
        process.env.OTP_ALGORITHM = undefined;
        process.env.OTP_DIGITS = undefined;
        process.env.OTP_PERIOD = undefined;
  ...
  describe("🔐 verifyOtpSetup", () => {
    ...
    describe("❌ Validation Errors", () => {
      ...
      it("should handle invalid OTP_DIGITS environment variable", async () => {
        ...
        expect(result.error).toBe(
          "Error interno en la configuración de OTP. Contacte al administrador.",
        );
        expect(mockConsole.error).toHaveBeenCalledWith(
    ...
    describe("❌ Database Errors", () => {
      it("should handle database errors during verification", async () => {
        ...
        expect(mockConsole.error).toHaveBeenCalledWith("Error verifying OTP setup:", dbError);
      });
