...
export interface AssociateBrandState { success: boolean; error?: string | null; organizationBrandId?: number | null; message?: string | null; }

export async function associateOrganizationBrand(prevState: AssociateBrandState | null, formData: FormData): Promise<AssociateBrandState> {
    ...
    if (!validatedFields.success) {
        ...
        const errors = Object.keys(fieldErrors).length > 0 
            ? Object.entries(fieldErrors).map(([f, m]: [string, string[] | undefined]) => `${f}: ${(m ?? []).join(',')}`).join('; ')
...
export interface UpdateAssociationState { success: boolean; error?: string | null; }

export async function updateOrganizationBrandAssociation(prevState: UpdateAssociationState | null, formData: FormData): Promise<UpdateAssociationState> {
    const organizationId = await getOrganizationIdFromSession();
    if (!organizationId) return { success: false, error: "Usuario no autenticado o sin organización." };
    ...
    try {
        ...
        const association = await prisma.organizationBrand.findUnique({ where: { id: organizationBrandId } });
        if (!association || association.organizationId !== organizationId) {
            return { success: false, error: "Asociación no encontrada o no pertenece a tu organización." };
        }
...
export interface DissociateBrandState { success: boolean; error?: string | null; }

export async function dissociateOrganizationBrand(prevState: DissociateBrandState | null, formData: FormData): Promise<DissociateBrandState> {
    const organizationId = await getOrganizationIdFromSession();
    if (!organizationId) return { success: false, error: "Usuario no autenticado o sin organización." };
    ...
    try {
         // Verificar que la asociación pertenece a la organización actual
         const associationToDelete = await prisma.organizationBrand.findUnique({ where: { id: organizationBrandId } });
...
export interface UpdateBrandOrderState { success: boolean; error?: string | null; }

export async function updateOrganizationBrandsOrder(prevState: UpdateBrandOrderState | null, orderedAssociations: { id: number; order: number }[]): Promise<UpdateBrandOrderState> {
    const organizationId = await getOrganizationIdFromSession();
    if (!organizationId) return { success: false, error: "Usuario no autenticado o sin organización." };

    const validationResult = updateOrgBrandOrderSchema.safeParse(orderedAssociations);
    if (!validationResult.success) {
        const errors = validationResult.error.flatten().formErrors.join('; ') || "Invalid order data provided.";
    ...
    try {
        ...
        if(existingAssociations.length !== associationIds.length) {
            return { success: false, error: "Error: Una o más asociaciones no pertenecen a tu organización." };
        }
...
export interface RenameBrandState { success: boolean; error?: string | null; newOrganizationBrandId?: number | null; message?: string | null; }
...
export async function renameBrandByDuplication(prevState: RenameBrandState | null, formData: FormData): Promise<RenameBrandState> {
    const organizationId = await getOrganizationIdFromSession();
    if (!organizationId) return { success: false, error: "Usuario no autenticado o sin organización." };
    ...
    const normalizedNewName = newBrandName.charAt(0).toUpperCase() + newBrandName.slice(1).toLowerCase();
    
    console.log(`[renameBrandByDuplication N:M] Iniciando para oldAssocId: ${oldOrganizationBrandId}, newBrandName: "${normalizedNewName}", orgId: ${organizationId}`); 

    try {
        const result = await prisma.$transaction(async (tx) => {
            console.log(`[renameBrandByDuplication N:M] Dentro de transacción para oldAssocId: ${oldOrganizationBrandId}`); 
            ...
            if (!newBrand) {
                ...
                console.log(`[renameBrandByDuplication N:M] Nueva marca global creada: ID ${newBrand.id}, Nombre ${newBrand.name}`);
            } else {
                 console.log(`[renameBrandByDuplication N:M] Marca global encontrada: ID ${newBrand.id}, Nombre ${newBrand.name}`);
            ...
            const newAssociation = await tx.organizationBrand.create({
             ...
             console.log(`[renameBrandByDuplication N:M] Nueva asociación creada: ID ${newAssociation.id} (Org: ${organizationId}, Brand: ${newBrandId})`);

            // 4. Eliminar la ANTIGUA asociación de OrganizationBrand
            console.log(`[renameBrandByDuplication N:M] Eliminando antigua asociación ID: ${oldOrganizationBrandId}`);
...
export interface CreateModelState { success: boolean; error?: string | null; modelId?: number | null; orgModelConfigId?: number | null; message?: string | null; }

export async function addModelToOrganizationBrand(prevState: CreateModelState | null, formData: FormData): Promise<CreateModelState> {
    const organizationId = await getOrganizationIdFromSession();
    if (!organizationId) return { success: false, error: "Usuario no autenticado o sin organización." };
    ...
    try {
        ...
        if (!hasPermission) return { success: false, error: "No tienes permiso para añadir modelos a esta marca." };
            ...
            if (existingModel) {
                ...
                if (existingConfig) {
                    ...
                    if (existingConfig.isVisible) {
                        ...
                        throw new Error(`El modelo "${normalizedName}" ya está activo para esta marca en tu organización.`);
                    } else {
                        ...
                        return { modelId: modelId, orgModelConfigId: updatedConfig.id, reactivated: true, configCreated: false, modelCreated: false, brandName: existingModel.name }; // Añadir brandName para mensajes
                    }
                } else {
                    ...
                    return { modelId: modelId, orgModelConfigId: newOrgModelConfig.id, reactivated: false, configCreated: true, modelCreated: false, brandName: existingModel.name };
                }
            } else {
                ...
                return { modelId: newModelId, orgModelConfigId: newOrgModelConfig.id, reactivated: false, configCreated: false, modelCreated: true, brandName: brand?.name ?? '' };
            }
        }); // Fin transacción
...
export interface UpdateModelState { success: boolean; error?: string | null; newOrgModelConfigId?: number | null; message?: string | null; }

// Mantener schema updateModelSchema para la validación inicial de FormData

export async function updateOrganizationModel(prevState: UpdateModelState | null, formData: FormData): Promise<UpdateModelState> {
    const organizationId = await getOrganizationIdFromSession();
    if (!organizationId) return { success: false, error: "Usuario no autenticado o sin organización." };
    ...
    console.log(`[replaceModel N:M] Iniciando para oldModelId: ${oldModelId}, newName: "${normalizedNewName}", brandId: ${brandId}, orgId: ${organizationId}`);
    ...
    try {
        const result = await prisma.$transaction(async (tx) => {
            ...
            // 1. Verificar permiso de la org sobre la Marca
            const hasPermission = await checkBrandPermission(organizationId, brandId);
            if (!hasPermission) throw new Error("No tienes permiso para modificar modelos de esta marca.");
            ...
            if (!oldConfig) throw new Error("Configuración del modelo original no encontrada para esta organización.");
            if (!oldConfig.isVisible) throw new Error("No se puede renombrar un modelo que ya está oculto.");
            const oldModelName = oldConfig.model.name;
            const originalOrder = oldConfig.order;
            console.log(`[replaceModel N:M] Config antigua encontrada: ID ${oldConfig.id}, Order ${originalOrder}, Visible ${oldConfig.isVisible}`);
            ...
            if (existingNewConfig) {
                 console.log(`[replaceModel N:M] Config para NUEVO modelo encontrada: ID ${existingNewConfig.id}, Visible ${existingNewConfig.isVisible}`);
                if (existingNewConfig.isVisible) {
                    throw new Error(`El modelo "${normalizedNewName}" ya está activo para esta marca en tu organización.`);
                }
                ...
                newOrgModelConfigId = updatedConfig.id;
                 console.log(`[replaceModel N:M] Config para NUEVO modelo REACTIVADA: ID ${newOrgModelConfigId}`);
            } else {
                ...
                newOrgModelConfigId = createdConfig.id;
                 console.log(`[replaceModel N:M] Config para NUEVO modelo CREADA: ID ${newOrgModelConfigId}`);
            }
        ...
        const message = error instanceof Error ? error.message : "Error inesperado.";
         ...
         if (message.includes("ya está activo") || message.includes("igual al nombre actual") || message.includes("modelo que ya está oculto")) {
            return { success: false, error: message };
         }
...
export interface SetModelVisibilityState { success: boolean; error?: string | null; }
...
export async function setOrganizationModelVisibility(prevState: SetModelVisibilityState | null, formData: FormData): Promise<SetModelVisibilityState> {
    const organizationId = await getOrganizationIdFromSession();
    if (!organizationId) return { success: false, error: "Usuario no autenticado o sin organización." };
    ...
    try {
        ...
        const model = await prisma.model.findUnique({ 
            where: { id: modelId }, select: { brandId: true } 
        });
        ...
        await prisma.organizationModelConfig.upsert({
            ...
            update: { // Qué actualizar si existe
                isVisible: isVisible
            },
...
export interface UpdateModelOrderState { success: boolean; error?: string | null; }

export async function updateOrganizationModelsOrder(prevState: UpdateModelOrderState | null, data: { modelOrders: { modelId: number; order: number }[], brandId: number }): Promise<UpdateModelOrderState> {
    const organizationId = await getOrganizationIdFromSession();
    if (!organizationId) return { success: false, error: "Usuario no autenticado o sin organización." };
    ...
    if (!validatedData.success) {
        const errors = validatedData.error.flatten().formErrors.join('; ') || "Invalid order data provided.";
    ...
    console.log(`[updateOrgModelsOrder] Validated - OrgId: ${organizationId}, BrandId: ${brandId}, ModelOrders:`, modelOrders); // LOG DATOS VALIDADOS

    try {
        // 1. Verificar permiso de la org sobre la marca
        const hasPermission = await checkBrandPermission(organizationId, brandId);
        if (!hasPermission) {
            console.error(`[updateOrgModelsOrder] Permission Denied: Org ${organizationId} for Brand ${brandId}`); // LOG ERROR PERMISO
        ...
        const existingModels = await prisma.model.findMany({
            where: { id: { in: modelIds }, brandId: brandId }, select: { id: true }
        });
        if (existingModels.length !== modelIds.length) {
             console.error(`[updateOrgModelsOrder] Model Mismatch: Found ${existingModels.length}/${modelIds.length} models for Brand ${brandId}`); // LOG ERROR MODELOS
            return { success: false, error: "Error: Uno o más modelos no pertenecen a la marca especificada." };
        }
        ...
        const updates = modelOrders.map(item => {
            console.log(` -> Updating OrgModelConfig: OrgId=${organizationId}, ModelId=${item.modelId} to Order=${item.order}`); // LOG CADA UPDATE
        ...
        console.log(`[updateOrgModelsOrder] Total records updated: ${updatedCount} (Expected: ${modelOrders.length})`); // LOG CONTEO
