...
export async function getSuppliersReport(filters: SuppliersReportFilters) {
  ...
  const whereConditions = {
    organizationId,
    ...(supplierId && { id: parseInt(supplierId) }),
  };
  ...
  const purchasesBySupplier = suppliersData.reduce((acc, supplier) => {
  ...
  const purchasesByMonth = suppliersData.reduce((acc, supplier) => {
    supplier.motorcycles.forEach(motorcycle => {
  ...
  return {
    ...
    supplierDetails: suppliersData.map(supplier => ({
      ...
      totalPurchases: supplier.motorcycles.reduce((acc, m) => {
