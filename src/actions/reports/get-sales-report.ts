...
export async function getSalesReport(dateRange?: { from?: Date; to?: Date }): Promise<SalesReport> {
    ...
    const salesBySeller = sales.reduce((acc, sale) => {
        ...
        if (sale.costPrice) {
            acc[sellerId].profit[currency] = (acc[sellerId].profit[currency] || 0) + 
                (sale.retailPrice - sale.costPrice);
        }
    ...
    const summary = sales.reduce((acc, sale) => {
        ...
        if (sale.costPrice) {
            acc.totalProfit[currency] = (acc.totalProfit[currency] || 0) + 
                (sale.retailPrice - sale.costPrice);
        }
    ...
    const averagePrice = Object.entries(summary.totalRevenue).reduce((acc, [currency, total]) => {
