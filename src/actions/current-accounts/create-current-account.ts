...
import { z } from 'zod';
import { Prisma, type CurrentAccount as PrismaCurrentAccount, type PaymentFrequency } from '@prisma/client';
...
function calculatePaymentDates(startDate: Date, numberOfInstallments: number, frequency: PaymentFrequency) {
  ...
  for (let i = 0; i < numberOfInstallments; i++) {
    if (i > 0) { // For payments after the first one
        switch (frequency) {
            case 'WEEKLY':
    ...
    if (i === 0) { // First payment due date is the start date itself or first calculated if installments > 1
...
function calculateInstallmentWithInterest(
    ...
    if (ratePerPeriod === 0 && annualRateDecimal !== 0) { // Si la tasa anual no es cero pero la tasa por período sí (muchos períodos)
        return principal / installments; // Evitar división por cero si el interés es insignificante por período
    }
    if (ratePerPeriod === -1) { // Evita denominador 0 si tasa es -100% por período
...
export async function createCurrentAccount(
  ...
  console.log("🔍 [createCurrentAccount] Iniciando creación de cuenta corriente con datos:", JSON.stringify(input, null, 2));
  
  try {
    const validatedInput = createCurrentAccountSchema.safeParse(input);
    if (!validatedInput.success) {
      console.error("❌ [createCurrentAccount] Error en la validación del esquema:", validatedInput.error);
      return {
        success: false,
    ...
    console.log("✅ [createCurrentAccount] Cliente encontrado:", clientExists.firstName, clientExists.lastName);

    console.log("🔍 [createCurrentAccount] Verificando existencia de la motocicleta con ID:", motorcycleId);
    ...
    console.log("✅ [createCurrentAccount] Motocicleta encontrada, marca/modelo:", motorcycleExists.brandId, motorcycleExists.modelId);

    const remainingAmount = totalAmount - downPayment;
    if (remainingAmount < 0) {
        console.error("❌ [createCurrentAccount] Error de pago inicial:", { totalAmount, downPayment, remainingAmount });
    ...
    if (Math.abs(remainingAmount - expectedTotalFromInstallments) > 0.01) { // Allow for small rounding differences
        // This could be a warning or an error depending on business rules
        console.warn("⚠️ [createCurrentAccount] Advertencia: La suma de las cuotas no coincide exactamente con el monto restante a financiar:", {
          remainingAmount,
    ...
    console.log("🔍 [createCurrentAccount] Intentando crear registro en la base de datos con organizationId:", organizationId);
    ...
    console.log("✅ [createCurrentAccount] Cuenta corriente creada exitosamente:", newCurrentAccount.id);
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
        ...
        return { success: false, error: "Error de base de datos al crear la cuenta: " + error.message };
    }
    return { success: false, error: "Error desconocido al crear la cuenta corriente: " + (error instanceof Error ? error.message : String(error)) };
  }
} 