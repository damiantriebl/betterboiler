...
export async function deletePettyCashMovement(
    ...
    console.log("ℹ️ Session data from getOrganizationIdFromSession (in movement delete):", sessionData);

    if (sessionData.error || !sessionData.organizationId || !sessionData.userRole || !sessionData.userEmail) {
        console.error("🚫 Error: Invalid session data or missing essential fields (in movement delete).", sessionData);
    ...
    console.log("✅ Session data valid (in movement delete):", { organizationId: sessionData.organizationId, userRole: sessionData.userRole, userEmail: sessionData.userEmail });

    const { organizationId, userRole, userEmail } = sessionData;

    const allowedRoles = ["admin", "root", "cash-manager"];
    if (!allowedRoles.includes(userRole)) {
        console.error(`🚫 Error: User role '${userRole}' not in allowedRoles (in movement delete):`, allowedRoles);
    ...
    try {
        console.log("⏳ Fetching organization settings for organizationId (in movement delete):", organizationId);
        ...
        if (!organizationSettings) {
            console.error("🚫 Error: Organization settings not found for organizationId (in movement delete):", organizationId);
        ...
        if (organizationSettings.secureModeEnabled) {
            ...
            if (!organizationSettings.otpVerified || !organizationSettings.otpSecret) {
                console.error("🚫 Error: Secure mode enabled but OTP not configured/verified in organization (in movement delete).", organizationSettings);
            ...
            if (!otpToken) {
                console.error("🚫 Error: Secure mode enabled, but no otpToken provided with params (in movement delete):", params);
            ...
            if (delta === null) {
                console.error("🚫 Error: Invalid or expired OTP token (in movement delete).", { otpToken, delta });
        ...
        if (!existingSpend) {
            console.error("🚫 Error: Spend not found or does not belong to organization. Movement ID:", movementId, "Org ID:", organizationId);
