...
export async function deletePettyCashDeposit(
  ...
  try {
    ...
    if (!organizationSettings) {
      console.error(
        "🚫 Error: Organization settings not found for organizationId:",
        organizationId,
      );
    ...
    if (organizationSettings.secureModeEnabled) {
      ...
      if (!organizationSettings.otpVerified || !organizationSettings.otpSecret) {
        console.error(
          "🚫 Error: Secure mode enabled but OTP not configured/verified in organization.",
      ...
      if (!otpToken) {
        console.error(
          "🚫 Error: Secure mode enabled, but no otpToken provided with params:",
          params,
        );
        return {
          success: false,
    ...
    console.log(
      "⏳ Checking if deposit exists and for associated withdrawals. Deposit ID:",
      depositId,
    );
    ...
    console.log(
      "ℹ️ Existing deposit data:",
      existingDeposit
        ? { id: existingDeposit.id, withdrawalsCount: existingDeposit.withdrawals.length }
        : null,
    );

    if (!existingDeposit) {
      console.error(
        "🚫 Error: Deposit not found or does not belong to organization. Deposit ID:",
        depositId,
        "Org ID:",
        organizationId,
      );
    ...
    if (existingDeposit.withdrawals && existingDeposit.withdrawals.length > 0) {
      console.error(
        "🚫 Error: Cannot delete deposit with associated withdrawals. Withdrawals count:",
        existingDeposit.withdrawals.length,
      );
      return {
        success: false,
        error:
