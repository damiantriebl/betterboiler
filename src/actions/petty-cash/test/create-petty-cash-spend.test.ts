...
describe('createPettyCashSpendWithTicket', () => {
  ...
  describe('Validaciones de negocio', () => {
    ...
    it('debería fallar cuando el retiro ya está justificado', async () => {
      ...
      expect(result.message).toBe('Este retiro ya ha sido completamente justificado.');
        ...
        it('debería fallar cuando se excede el monto del retiro', async () => {      mockPrismaTransaction.mockImplementation(async (callback: (tx: any) => Promise<any>) => {        const tx = {          pettyCashWithdrawal: {            findUnique: vi.fn().mockResolvedValue(mockWithdrawal),          },          pettyCashSpend: {            create: vi.fn().mockImplementation(() => {              throw new Error('El monto justificado excede el monto entregado en el retiro.');            }),          },        };        return await callback(tx);      });            const formData = createValidFormData({ amount: '4000' });      const result = await createPettyCashSpendWithTicket(initialState, formData);      expect(result.status).toBe('error');      expect(result.message).toBe('El monto justificado excede el monto entregado en el retiro.');    });
  });
  ...
  describe('Cache Revalidation', () => {
    ...
    it('no debería revalidar cuando hay errores de validación', async () => {      const formData = createValidFormData({ amount: '-500' });      await createPettyCashSpendWithTicket(initialState, formData);      expect(mockRevalidatePath).not.toHaveBeenCalled();    });  });});
