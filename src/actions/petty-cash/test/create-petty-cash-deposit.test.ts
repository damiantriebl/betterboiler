...
describe("Create Petty Cash Deposit", () => {
  ...
  const createValidFormData = (overrides: Record<string, string> = {}) => {
    ...
    formData.append("description", overrides.description ?? "Depósito de prueba");
    formData.append("amount", overrides.amount ?? "1000");
  ...
  describe("❌ Manejo de Errores de Base de Datos", () => {
    it("debería manejar errores de base de datos conocidos", async () => {
      ...
      expect(mockConsole.error).toHaveBeenCalledWith("Error creating petty cash deposit:", dbError);
    });
