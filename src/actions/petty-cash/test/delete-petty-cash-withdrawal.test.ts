...
describe('Delete Petty Cash Withdrawal', () => {
  ...
  describe('❌ Manejo de Errores de Retiro', () => {
    ...
    it('debería fallar cuando el retiro tiene movimientos asociados', async () => {
      ...
      // Assert
      expect(result.success).toBe(false);
      expect(result.error).toBe('No se puede eliminar el retiro porque tiene gastos asociados. Elimine los gastos primero.');
    });
  ...
  describe('🔒 Modo Seguro con OTP', () => {
    ...
    it('debería fallar cuando modo seguro está activado pero OTP no está configurado', async () => {
      ...
      // Act
      const result = await deletePettyCashWithdrawal({ withdrawalId: 'withdrawal-1', otpToken: '123456' });
    ...
    it('debería fallar con token OTP de formato inválido', async () => {
      ...
      // Act
      const result = await deletePettyCashWithdrawal({ withdrawalId: 'withdrawal-1', otpToken: 'abc123' });
    ...
    it('debería fallar con token OTP muy corto', async () => {
      ...
      // Act
      const result = await deletePettyCashWithdrawal({ withdrawalId: 'withdrawal-1', otpToken: '123' });
