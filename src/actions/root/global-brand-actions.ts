...
import { createBrandSchema, updateBrandSchema, updateBrandsOrderSchema } from "@/zod/brand-schemas";
import { createModelSchema, updateModelSchema, updateModelsOrderSchema } from "@/zod/model-schemas";
...
export async function getRootBrands() {
  try {
    ...
    const brandsWithFileStatus = brands.map((brand) => {
      const hasPhoto = brand.models.some(
        (model) => model.imageUrl || model.files.some((file) => file.type === "image"),
      );
      const hasTechnicalSheet = brand.models.some(
        (model) => model.specSheetUrl || model.files.some((file) => file.type === "spec_sheet"),
      );
...
export async function createRootModel(
  ...
  try {
    ...
    if (existing)
      return { success: false, error: `El modelo "${name}" ya existe para esta marca.` };
...
export async function updateRootBrand(
  ...
  try {
    ...
    return {
      success: true,
      message: `Marca "${updatedBrand.name}" actualizada.`,
      data: updatedBrand,
    };
  } catch (error) {
...
export async function updateRootModel(
  ...
  try {
    ...
    if (existing)
      return {
        success: false,
        error: `Ya existe otro modelo con el nombre "${name}" para esta marca.`,
      };
    ...
    return {
      success: true,
      message: `Modelo "${updatedModel.name}" actualizado.`,
      data: updatedModel,
    };
  } catch (error) {
...
export async function updateRootBrandsOrder(
  ...
  try {
    ...
    return handlePrismaError(
      error,
      "Error del servidor durante la validación del orden de marcas.",
    );
  }
}

export async function updateRootModelsOrder(orderPayload: {
  brandId: number;
  modelOrders: { modelId: number; order: number }[];
}): Promise<BatchActionState> {
  ...
  // If an 'order' field exists, uncomment the 'data' part in the transaction.
  console.log(
    `Attempting to update model order for brand ${brandId} (validation only):`,
    modelOrders,
  );
  ...
  try {
    ...
    return handlePrismaError(
      error,
      "Error del servidor durante la validación del orden de modelos.",
    );
  }
}
