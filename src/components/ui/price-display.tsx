    ...
    if (!mounted) {
        return (
            <div className={cn("flex flex-col", sizeClasses.container, className)}>
                <div>
                    <span className={cn(sizeClasses.value, "text-green-600")}>{formatPrice(retailPrice, currency)}</span>
                </div>
            </div>
        );
    }
        ...
        <div className={cn("flex flex-col", sizeClasses.container, className)}>
            {showCost() && costPrice !== undefined && costPrice !== null && (
                <div>
                    ...
                    <span className={cn(sizeClasses.value, "text-base")}>{formatPrice(costPrice, currency)}</span>
                </div>
            )}

            {showWholesale() && wholesalePrice !== undefined && wholesalePrice !== null && (
                <div>
                    <span className={cn("text-muted-foreground text-base", sizeClasses.label)}>Mayorista:</span>{" "}
                    <span className={cn(sizeClasses.value, "text-base")}>{formatPrice(wholesalePrice, currency)}</span>
                </div>
            )}

            {showRetail() && (
                <div>
                    <span className={cn("text-muted-foreground text-base", sizeClasses.label)}>Minorista:</span>{" "}
                    <span className={cn(sizeClasses.value, "text-green-600 text-base")}>{formatPrice(retailPrice, currency)}</span>
                </div>
            )}
        </div>
    );
