...
import { type Payment, type PaymentFrequency, type CurrentAccountStatus } from '@prisma/client';
...
function calculateFrenchAmortizationScheduleForPdf(
  ...
  for (let i = 1; i <= numberOfInstallments; i++) {
    ...
    if (i === numberOfInstallments) {
      ...
      finalInstallmentAmountForEntry = Math.ceil(currentCapital + interest) < fixedInstallment ? fixedInstallment : Math.ceil(currentCapital + interest);
      if (finalInstallmentAmountForEntry - interest > currentCapital) amortizationAmount = currentCapital;
...
function generateInstallmentsForPdf(account: CurrentAccountForReport): PdfInstallmentInfo[] {
  ...
  for (let i = 0; i < account.numberOfInstallments; i++) {
    ...
    switch (account.paymentFrequency) {
      case "WEEKLY": dueDate.setDate(startDate.getDate() + 7 * i); break;
      case "BIWEEKLY": dueDate.setDate(startDate.getDate() + 14 * i); break;
    ...
    const normalPayment = paymentsForThis.find(p => p.installmentVersion !== 'D' && p.installmentVersion !== 'H');
    ...
    if (!normalPayment || isAnnulledAndThusPendingAgain) {
      if (planEntry) {
        ...
        if (!normalPayment || isAnnulledAndThusPendingAgain) {
          const existingPending = generatedInstallments.find(gi => gi.number === installmentNumber && !gi.isPaid && !gi.installmentVersion);
          if (!existingPending) { // Avoid adding duplicate pending if already added above implicitly
            generatedInstallments.push({
              number: installmentNumber,
              dueDate,
  ...
  return generatedInstallments.sort((a, b) => {
    ...
    const getOrderScore = (inst: PdfInstallmentInfo) => {
      if (inst.isPaid && inst.installmentVersion !== 'D' && inst.installmentVersion !== 'H') return 0; // Normal Paid
...
const CurrentAccountReportDocument: React.FC<CurrentAccountReportDocumentProps> = ({ account }) => {
  ...
  const totalAmountOfAllCalculatedInstallments = amortizationPlan.reduce((sum, entry) => sum + Math.ceil(entry.calculatedInstallmentAmount), 0);
  ...
  const actualInstallmentPaymentsAmount = account.payments
    .filter(p => p.installmentVersion !== 'D' && p.installmentVersion !== 'H' && p.installmentNumber !== null && p.installmentNumber > 0)
  ...
  const frequencyMap: Record<string, string> = {
    WEEKLY: "Semanal", BIWEEKLY: "Quincenal", MONTHLY: "Mensual", QUARTERLY: "Trimestral", ANNUALLY: "Anual",
  };
  const statusTranslations: Record<string, string> = {
    ACTIVE: "Activa", PAID_OFF: "Pagada", OVERDUE: "Vencida", DEFAULTED: "Impago", CANCELLED: "Cancelada",
  };
  ...
  return (
    <Document title={`Reporte Cuenta Corriente - ${client.firstName} ${client.lastName} `} author={account.organization?.name || 'Sistema'}>
      <Page size="A4" style={styles.page}>
        ...
        {account.organization && (
          <View style={styles.section}>
            ...
            <Text style={styles.text}><Text style={styles.label}>Nombre:</Text> {account.organization.name}</Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.gridContainer}>
            <Text style={styles.gridItem}><Text style={styles.label}>Nombre:</Text> {client.firstName} {client.lastName}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Email:</Text> {client.email}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>ID Fiscal (CUIT/CUIL):</Text> {client.taxId}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Teléfono:</Text> {client.phone || 'N/A'}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Móvil:</Text> {client.mobile || 'N/A'}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Dirección:</Text> {client.address || 'N/A'}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Estado IVA:</Text> {client.vatStatus || 'N/A'}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Tipo Cliente:</Text> {client.type === 'company' ? 'Empresa' : 'Particular'}</Text>
          </View>
          {client.notes && (
        ...
        {motorcycle && (
          <View style={styles.section}>
            ...
            <View style={styles.gridContainer}>
              <Text style={styles.gridItem}><Text style={styles.label}>Marca:</Text> {motorcycle.brand?.name || 'N/A'}</Text>
              <Text style={styles.gridItem}><Text style={styles.label}>Modelo:</Text> {motorcycle.model?.name || 'N/A'}</Text>
              <Text style={styles.gridItem}><Text style={styles.label}>Año:</Text> {motorcycle.year}</Text>
              <Text style={styles.gridItem}><Text style={styles.label}>Color:</Text> {motorcycle.color?.name || 'N/A'}</Text>
              <Text style={styles.gridItem}><Text style={styles.label}>Nº Chasis:</Text> {motorcycle.chassisNumber}</Text>
              <Text style={styles.gridItem}><Text style={styles.label}>Nº Motor:</Text> {motorcycle.engineNumber || 'N/A'}</Text>
              <Text style={styles.gridItem}><Text style={styles.label}>Precio Venta:</Text> {formatCurrencyForPdf(motorcycle.retailPrice, account.currency || '$')}</Text>
              <Text style={styles.gridItem}><Text style={styles.label}>Sucursal:</Text> {motorcycle.branch?.name || 'N/A'}</Text>
            </View>
            {seller && (
              <View style={styles.subSection}>
                <Text style={styles.label}>Vendido por:</Text>
                <Text style={styles.text}>{seller.name} ({seller.email})</Text>
              </View>
            )}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.gridContainer}>
            <Text style={styles.gridItem}><Text style={styles.label}>ID Cuenta:</Text> {account.id}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Estado Cuenta:</Text> {statusTranslations[account.status] || account.status}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Monto Total (c/Int.):</Text> {formatCurrencyForPdf(grandTotalObligation, account.currency || '$')}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Pago Inicial:</Text> {formatCurrencyForPdf(account.downPayment, account.currency || '$')}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Monto Financiado:</Text> {formatCurrencyForPdf(financialPrincipal, account.currency || '$')}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Tasa Interés Anual:</Text> {account.interestRate ?? 0}%</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Nº Cuotas:</Text> {account.numberOfInstallments}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Frecuencia:</Text> {frequencyMap[account.paymentFrequency] || account.paymentFrequency}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Fecha Inicio (1er Venc.):</Text> {formatDateForPdf(account.startDate)}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Monto Total Pagado:</Text> {formatCurrencyForPdf(totalEffectivelyPaid, account.currency || '$')}</Text>
            <Text style={styles.gridItem}><Text style={styles.label}>Saldo Pendiente (c/Int.):</Text> {formatCurrencyForPdf(uiRemainingAmountDisplay, account.currency || '$')}</Text>
          </View>
          {account.notes && (
        ...
        <View style={styles.section}>
          ...
          <View style={styles.table}>
            ...
            <View style={styles.tableRow} fixed>
              ...
              <Text style={{ ...styles.tableColHeader, width: colWidths.dueDate }}>Vencimiento</Text>
              <Text style={{ ...styles.tableColHeader, width: colWidths.capStart }}>Cap. Inicio</Text>
            ...
            {installmentsForPdf.map((inst, index) => (
              <View key={inst.paymentId || `inst-${inst.number}-${index}`} style={styles.tableRow} wrap={false}>
                <Text style={{ ...styles.tableCol, ...styles.tableCellCenter, width: colWidths.num }}>{inst.number}{inst.installmentVersion && inst.installmentVersion !== 'Original (Anulado)' ? String(inst.installmentVersion).toUpperCase() : ''}</Text>
                <Text style={{ ...styles.tableCol, width: colWidths.dueDate }}>{formatDateForPdf(inst.dueDate)}</Text>
                <Text style={{ ...styles.tableCol, ...styles.tableCellRight, width: colWidths.capStart }}>{inst.capitalAtPeriodStart !== undefined ? formatCurrencyForPdf(inst.capitalAtPeriodStart, '') : '-'}</Text>
                <Text style={{ ...styles.tableCol, ...styles.tableCellRight, width: colWidths.amort }}>{inst.amortization !== undefined && inst.installmentVersion !== 'D' && inst.installmentVersion !== 'H' ? formatCurrencyForPdf(inst.amortization, '') : '-'}</Text>
                <Text style={{ ...styles.tableCol, ...styles.tableCellRight, width: colWidths.interest }}>{inst.interestForPeriod !== undefined && inst.installmentVersion !== 'D' && inst.installmentVersion !== 'H' ? formatCurrencyForPdf(inst.interestForPeriod, '') : '-'}</Text>
                <Text style={{ ...styles.tableCol, ...styles.tableCellRight, fontWeight: 'bold', width: colWidths.cuota }}>
                  {formatCurrencyForPdf(
                    (inst.installmentVersion === 'H' ? -1 : 1) *
                    (inst.isPaid && inst.originalPaymentAmount !== undefined ? inst.originalPaymentAmount : inst.calculatedInstallmentAmount ?? inst.amount),
                    account.currency || '$'
                  )}
                </Text>
                <Text style={{ ...styles.tableCol, width: colWidths.status }}>
                  {inst.installmentVersion === 'D' ? 'Anulación (D)' :
                    inst.installmentVersion === 'H' ? 'Anulación (H)' :
                      inst.isPaid ? `Pagada ${inst.paymentDate ? formatDateForPdf(inst.paymentDate) : ''}` :
                        'Pendiente'}
                </Text>
              </View>
            ))}
        ...
        <Text style={styles.footer} fixed>
          Reporte generado el {reportDate} por {account.organization?.name || 'Sistema'}. Página {'{{pageNumber}}'} de {'{{totalPages}}'}
        </Text>
      </Page>
    </Document>
  );
