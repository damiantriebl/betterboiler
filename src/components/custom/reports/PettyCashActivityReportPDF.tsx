...
const PettyCashActivityReportPDF: React.FC<PettyCashActivityReportPDFProps> = ({
  data,
  fromDate,
  toDate,
}) => {
  ...
  return (
    <Document
      ...
      title={`Reporte de Actividad de Caja Chica ${formatDateShort(fromDate)} - ${formatDateShort(toDate)}`}
    >
        ...
        {data.map((deposit) => (
          <View key={deposit.id} style={{ marginBottom: 15 }}>
            <Text style={styles.sectionTitle}>
              Depósito - {formatDate(deposit.date)} - Sucursal:{" "}
              {deposit.branch?.name || "Cuenta General"} - Monto: {formatCurrency(deposit.amount)}
            </Text>
            <View style={styles.itemDetail}>
              <Text>
                <Text style={styles.itemLabel}>Referencia: </Text>
                {deposit.reference || "-"}
              </Text>
            </View>
            <View style={styles.itemDetail}>
            ...
            {deposit.withdrawals.length > 0 ? (
              deposit.withdrawals.map((withdrawal) => (
                <View key={withdrawal.id} style={styles.indent1}>
                  ...
                  >
                    Retiro - {formatDate(withdrawal.date)} - Usuario: {withdrawal.userName} - Monto
                    Entregado: {formatCurrency(withdrawal.amountGiven)}
                  </Text>
                  <View style={styles.itemDetail}>
                    <Text>
                      <Text style={styles.itemLabel}>Justificado: </Text>
                  ...
                  {withdrawal.spends.length > 0 ? (
                    <View style={styles.indent2}>
                      <Text
                        style={{ ...styles.itemLabel, fontSize: 9, marginTop: 5, marginBottom: 3 }}
                      >
                        Gastos Asociados:
                      </Text>
                        ...
                        {withdrawal.spends.map((spend) => (
                          <View key={spend.id} style={styles.tableRow}>
                            <View style={{ ...styles.tableCol, width: "25%" }}>
                              <Text style={styles.textCenter}>{formatDateShort(spend.date)}</Text>
                            </View>
                            <View style={{ ...styles.tableCol, width: "25%" }}>
                              <Text>{spend.motive || "-"}</Text>
                            </View>
                            <View style={{ ...styles.tableCol, width: "35%" }}>
                              <Text>{spend.description}</Text>
                            </View>
                            <View style={{ ...styles.tableCol, width: "15%" }}>
                              <Text style={styles.textRight}>{formatCurrency(spend.amount)}</Text>
                            </View>
                          </View>
                        ))}
                      </View>
                    ...
                    <Text style={{ ...styles.indent2, fontSize: 8.5, fontStyle: "italic" }}>
              ...
              <Text style={{ ...styles.indent1, fontSize: 8.5, fontStyle: "italic" }}>
