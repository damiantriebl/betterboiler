...
async function seedBrandsAndModels() {
    ...
    console.log(`Seed de marcas y modelos completado: ${motos.length} marcas y ${modelosCreados} modelos creados.`);
}
...
async function seedGlobalColors() {
    ...
    console.log(`Seed de colores globales completado: ${coloresCreados} colores creados/actualizados.`);
}
...
async function seedPaymentMethods() {
    ...
    console.log(`Sincronizando ${DEFAULT_PAYMENT_METHODS.length} métodos de pago en la tabla PaymentMethod...`);
...
async function seedBanks() {
    ...
    if (existingCount > 0) {
        console.log(`⚠️ Ya existen ${existingCount} bancos en la base de datos. No se cargarán datos duplicados.`);
        return;
    }
...
async function seedCardTypesAndBankCards() {
    try {
        ...
        if (existingCardTypes === 0) {
            ...
            const cardTypes = [
                { name: "Visa", type: "credit", logoUrl: "https://placehold.co/50x30/blue/white?text=VISA" },
                { name: "Mastercard", type: "credit", logoUrl: "https://placehold.co/50x30/red/white?text=MC" },
                { name: "American Express", type: "credit", logoUrl: "https://placehold.co/50x30/green/white?text=AMEX" },
                { name: "Visa Débito", type: "debit", logoUrl: "https://placehold.co/50x30/blue/white?text=VISA-D" },
                { name: "Maestro", type: "debit", logoUrl: "https://placehold.co/50x30/red/white?text=MAESTRO" },
            ];
        ...
        if (!bankCardTableExists) {
            console.log("⚠️ La tabla BankCard no existe aún. Saltando seed de asociaciones banco-tarjeta.");
            return;
        }
        ...
        if (organizations.length === 0) {
            console.log("No hay organizaciones en la base de datos para asignar asociaciones banco-tarjeta.");
            return;
        }
        ...
        for (const org of organizations) {
            ...
            if (existingAssociations > 0) {
                console.log(`⚠️ Ya existen ${existingAssociations} asociaciones banco-tarjeta para esta organización.`);
                continue;
            }
            ...
            console.log(`✅ ${bankCardAssociations.length} asociaciones banco-tarjeta creadas para la organización ${org.id}!`);
        }
    } catch (error) {
...
async function seedBankingPromotions() {
    try {
        ...
        if (!tableExists) {
            console.log("⚠️ La tabla BankingPromotion no existe aún. Saltando seed de promociones bancarias.");
            return;
        }
        ...
        if (existingCount > 0) {
            console.log(`⚠️ Ya existen ${existingCount} promociones bancarias en la base de datos. No se cargarán datos de ejemplo.`);
            return;
        }
        ...
        for (const org of organizations) {
            ...
            if (methodsByType.transfer) {
                await prisma.bankingPromotion.create({
                    data: {
                        ...
                        description: "Pago por transferencia bancaria con 2% de recargo por gastos administrativos",
                        organizationId: org.id,
            ...
            if (methodsByType.credit && cards.length > 0 && banks.length > 0) {
                ...
                const bankProvinciaCard = cards.find(card =>
...
async function seedBankingPromotionsWithBankCards() {
    try {
        ...
        if (existingCount > 0) {
            console.log(`⚠️ Ya existen ${existingCount} promociones bancarias con BankCard en la base de datos.`);
            return;
        }
        ...
        for (const org of organizations) {
            ...
            let exampleBankCard = bankCards.find(bc =>
            ...
            if (exampleBankCard && methodsByType.credit) {
                ...
                console.log(`✅ Promoción con BankCard creada para ${exampleBankCard.cardType.name} de ${exampleBankCard.bank.name}`);
            }
        }
